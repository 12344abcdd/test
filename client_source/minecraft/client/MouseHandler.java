...
public class MouseHandler {
   ...
   private void onScroll(long l, double d, double e) {
      if (l == Minecraft.getInstance().getWindow().getWindow()) {
         boolean bl = (Boolean)this.minecraft.options.discreteMouseScroll().get();
         double f = (Double)this.minecraft.options.mouseWheelSensitivity().get();
         ...
         if (this.minecraft.getOverlay() == null) {
            if (this.minecraft.screen != null) {
               ...
               if (this.accumulatedScrollX != 0.0D && Math.signum(g) != Math.signum(this.accumulatedScrollX)) {
                  this.accumulatedScrollX = 0.0D;
               }

               if (this.accumulatedScrollY != 0.0D && Math.signum(h) != Math.signum(this.accumulatedScrollY)) {
   ...
   public void setup(long l) {
      InputConstants.setupMouseCallbacks(l, (lx, d, e) -> {
         this.minecraft.execute(() -> {
            this.onMove(lx, d, e);
         });
      }, (lx, i, j, k) -> {
         this.minecraft.execute(() -> {
            this.onPress(lx, i, j, k);
         });
      }, (lx, d, e) -> {
         this.minecraft.execute(() -> {
            this.onScroll(lx, d, e);
         });
      }, (lx, i, m) -> {
   ...
   public void handleAccumulatedMovement() {
      ...
      if (this.minecraft.isWindowActive()) {
         Screen screen = this.minecraft.screen;
         if (screen != null && this.minecraft.getOverlay() == null && (this.accumulatedDX != 0.0D || this.accumulatedDY != 0.0D)) {
   ...
   private void turnPlayer(double d) {
      double e = (Double)this.minecraft.options.sensitivity().get() * 0.6000000238418579D + 0.20000000298023224D;
      double f = e * e * e;
      double g = f * 8.0D;
      double j;
      double k;
