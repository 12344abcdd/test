...
public class QuickPlay {
   ...
   private static void joinSingleplayerWorld(Minecraft minecraft, String string) {
      if (!minecraft.getLevelSource().levelExists(string)) {
         ...
         minecraft.createWorldOpenFlows().openWorld(string, () -> {
   ...
   private static void joinRealmsWorld(Minecraft minecraft, RealmsClient realmsClient, String string) {
      ...
      try {
         ...
         minecraft.setScreen(new DisconnectedScreen(titleScreen, ERROR_TITLE, REALM_CONNECT, TO_TITLE));
         return;
      }

      RealmsServer realmsServer = (RealmsServer)realmsServerList.servers.stream().filter((realmsServerx) -> {
         return realmsServerx.id == l;
      ...
      if (realmsServer == null) {
         ...
         minecraft.setScreen(new RealmsLongRunningMcoTaskScreen(titleScreen, new LongRunningTask[]{getServerDetailsTask}));
      }
   }
}
