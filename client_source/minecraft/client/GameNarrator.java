...
public class GameNarrator {
   ...
   public void updateNarratorStatus(NarratorStatus narratorStatus) {
      ...
      if (this.narrator.active()) {
         if (narratorStatus == NarratorStatus.OFF) {
            ...
            SystemToast.addOrUpdate(toastComponent, SystemToast.SystemToastId.NARRATOR_TOGGLE, Component.translatable("narrator.toast.enabled"), narratorStatus.getName());
         }
      } else {
   ...
   public void checkStatus(boolean bl) {
      if (bl && !this.isActive() && !TinyFileDialogs.tinyfd_messageBox("Minecraft", "Failed to initialize text-to-speech library. Do you want to continue?\nIf this problem persists, please report it at bugs.mojang.com", "yesno", "error", true)) {
