...
public class OcelotModel<T extends Entity> extends AgeableListModel<T> {
   ...
   public static MeshDefinition createBodyMesh(CubeDeformation cubeDeformation) {
      ...
      partDefinition.addOrReplaceChild("tail2", CubeListBuilder.create().texOffs(4, 15).addBox(-0.5F, 0.0F, 0.0F, 1.0F, 8.0F, 1.0F, cubeDeformation2), PartPose.offset(0.0F, 20.0F, 14.0F));
   ...
   public void setupAnim(T entity, float f, float g, float h, float i, float j) {
      ...
      if (this.state != 3) {
         ...
         if (this.state == 2) {
            ...
            this.leftFrontLeg.xRot = Mth.cos(f * 0.6662F + 3.1415927F + 0.3F) * g;
            this.rightFrontLeg.xRot = Mth.cos(f * 0.6662F + 3.1415927F) * g;
            this.tail2.xRot = 1.7278761F + 0.31415927F * Mth.cos(f) * g;
         } else {
            this.leftHindLeg.xRot = Mth.cos(f * 0.6662F) * g;
            this.rightHindLeg.xRot = Mth.cos(f * 0.6662F + 3.1415927F) * g;
            this.leftFrontLeg.xRot = Mth.cos(f * 0.6662F + 3.1415927F) * g;
            this.rightFrontLeg.xRot = Mth.cos(f * 0.6662F) * g;
            if (this.state == 1) {
               this.tail2.xRot = 1.7278761F + 0.7853982F * Mth.cos(f) * g;
            } else {
