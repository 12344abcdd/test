...
public class EndermiteModel<T extends Entity> extends HierarchicalModel<T> {
   ...
   public void setupAnim(T entity, float f, float g, float h, float i, float j) {
      for(int k = 0; k < this.bodyParts.length; ++k) {
         this.bodyParts[k].yRot = Mth.cos(h * 0.9F + (float)k * 0.15F * 3.1415927F) * 3.1415927F * 0.01F * (float)(1 + Math.abs(k - 2));
         this.bodyParts[k].x = Mth.sin(h * 0.9F + (float)k * 0.15F * 3.1415927F) * 3.1415927F * 0.1F * (float)Math.abs(k - 2);
      }
