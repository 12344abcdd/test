...
public final class ModelPart {
   ...
   private void visit(PoseStack poseStack, ModelPart.Visitor visitor, String string) {
      if (!this.cubes.isEmpty() || !this.children.isEmpty()) {
         ...
         this.children.forEach((string2x, modelPart) -> {
            modelPart.visit(poseStack, visitor, string2 + string2x);
         });
         poseStack.popPose();
      }
   }
   ...
   private void compile(PoseStack.Pose pose, VertexConsumer vertexConsumer, int i, int j, int k) {
      ...
      while(var6.hasNext()) {
         ModelPart.Cube cube = (ModelPart.Cube)var6.next();
   ...
   public static class Cube {
      ...
      public void compile(PoseStack.Pose pose, VertexConsumer vertexConsumer, int i, int j, int k) {
         ...
         for(int var10 = 0; var10 < var9; ++var10) {
            ModelPart.Polygon polygon = var8[var10];
            Vector3f vector3f2 = pose.transformNormal(polygon.normal, vector3f);
            float f = vector3f2.x();
            float g = vector3f2.y();
            float h = vector3f2.z();
            ModelPart.Vertex[] var16 = polygon.vertices;
            int var17 = var16.length;

            for(int var18 = 0; var18 < var17; ++var18) {
               ModelPart.Vertex vertex = var16[var18];
