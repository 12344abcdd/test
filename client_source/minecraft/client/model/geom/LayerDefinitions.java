...
public class LayerDefinitions {
   ...
   public static Map<ModelLayerLocation, LayerDefinition> createRoots() {
      ...
      Type[] var24 = Type.values();
      int var25 = var24.length;

      for(int var26 = 0; var26 < var25; ++var26) {
         Type type = var24[var26];
         if (type == Type.BAMBOO) {
      ...
      WoodType.values().forEach((woodType) -> {
         builder.put(ModelLayers.createSignModelName(woodType), layerDefinition24);
      });
      LayerDefinition layerDefinition25 = HangingSignRenderer.createHangingSignLayer();
      WoodType.values().forEach((woodType) -> {
         builder.put(ModelLayers.createHangingSignModelName(woodType), layerDefinition25);
      });
      ImmutableMap<ModelLayerLocation, LayerDefinition> immutableMap = builder.build();
      List<ModelLayerLocation> list = (List)ModelLayers.getKnownLocations().filter((modelLayerLocation) -> {
         return !immutableMap.containsKey(modelLayerLocation);
