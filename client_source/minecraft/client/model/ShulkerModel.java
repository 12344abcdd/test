...
public class ShulkerModel<T extends Shulker> extends ListModel<T> {
   ...
   public void setupAnim(T shulker, float f, float g, float h, float i, float j) {
      ...
      float l = (0.5F + shulker.getClientPeekAmount(k)) * 3.1415927F;
      ...
      if (shulker.getClientPeekAmount(k) > 0.3F) {
         this.lid.yRot = m * m * m * m * 3.1415927F * 0.125F;
      } else {
         this.lid.yRot = 0.0F;
      }

      this.head.xRot = j * 0.017453292F;
      this.head.yRot = (shulker.yHeadRot - 180.0F - shulker.yBodyRot) * 0.017453292F;
   }
