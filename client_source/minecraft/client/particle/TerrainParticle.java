...
public class TerrainParticle extends TextureSheetParticle {
   ...
   static TerrainParticle createTerrainParticle(BlockParticleOption blockParticleOption, ClientLevel clientLevel, double d, double e, double f, double g, double h, double i) {
      ...
      return !blockState.isAir() && !blockState.is(Blocks.MOVING_PISTON) && blockState.shouldSpawnTerrainParticles() ? new TerrainParticle(clientLevel, d, e, f, g, h, i, blockState) : null;
   }
   ...
   public static class DustPillarProvider implements ParticleProvider<BlockParticleOption> {
      ...
      public Particle createParticle(BlockParticleOption blockParticleOption, ClientLevel clientLevel, double d, double e, double f, double g, double h, double i) {
         ...
         if (particle != null) {
            particle.setParticleSpeed(clientLevel.random.nextGaussian() / 30.0D, h + clientLevel.random.nextGaussian() / 2.0D, clientLevel.random.nextGaussian() / 30.0D);
