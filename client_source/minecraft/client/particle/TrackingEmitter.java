...
public class TrackingEmitter extends NoRenderParticle {
   ...
   public void tick() {
      for(int i = 0; i < 16; ++i) {
         ...
         if (!(d * d + e * e + f * f > 1.0D)) {
            double g = this.entity.getX(d / 4.0D);
            double h = this.entity.getY(0.5D + e / 4.0D);
            double j = this.entity.getZ(f / 4.0D);
            this.level.addParticle(this.particleType, false, g, h, j, d, e + 0.2D, f);
         }
      }
