...
public class BlockStatePredictionHandler implements AutoCloseable {
   ...
   public void retainKnownServerState(BlockPos blockPos, BlockState blockState, LocalPlayer localPlayer) {
      this.serverVerifiedStates.compute(blockPos.asLong(), (long_, serverVerifiedState) -> {
         return serverVerifiedState != null ? serverVerifiedState.setSequence(this.currentSequenceNr) : new BlockStatePredictionHandler.ServerVerifiedState(this.currentSequenceNr, blockState, localPlayer.position());
      });
   }

   public boolean updateKnownServerState(BlockPos blockPos, BlockState blockState) {
      BlockStatePredictionHandler.ServerVerifiedState serverVerifiedState = (BlockStatePredictionHandler.ServerVerifiedState)this.serverVerifiedStates.get(blockPos.asLong());
