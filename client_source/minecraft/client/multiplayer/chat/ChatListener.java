...
public class ChatListener {
   ...
   public void tick() {
      if (this.messageDelay != 0L) {
         if (Util.getMillis() >= this.previousMessageTime + this.messageDelay) {
            for(ChatListener.Message message = (ChatListener.Message)this.delayedMessageQueue.poll(); message != null && !message.accept(); message = (ChatListener.Message)this.delayedMessageQueue.poll()) {
            }
         }
   ...
   public void handlePlayerChatMessage(PlayerChatMessage playerChatMessage, GameProfile gameProfile, Bound bound) {
      boolean bl = (Boolean)this.minecraft.options.onlyShowSecureChat().get();
   ...
   public void handleChatMessageError(UUID uUID, Bound bound) {
   ...
   private boolean showMessageToPlayer(Bound bound, PlayerChatMessage playerChatMessage, Component component, GameProfile gameProfile, boolean bl, Instant instant) {
   ...
   private void logPlayerMessage(PlayerChatMessage playerChatMessage, Bound bound, GameProfile gameProfile, ChatTrustLevel chatTrustLevel) {
   ...
   public void handleSystemMessage(Component component, boolean bl) {
      if (!(Boolean)this.minecraft.options.hideMatchedNames().get() || !this.minecraft.isBlocked(this.guessChatUUID(component))) {
         if (bl) {
   ...
   static {
      CHAT_VALIDATION_ERROR = Component.translatable("chat.validation_error").withStyle(new ChatFormatting[]{ChatFormatting.RED, ChatFormatting.ITALIC});
   }
