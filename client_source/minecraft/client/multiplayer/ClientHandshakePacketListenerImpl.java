...
public class ClientHandshakePacketListenerImpl implements ClientLoginPacketListener {
   ...
   private void switchState(ClientHandshakePacketListenerImpl.State state) {
      ClientHandshakePacketListenerImpl.State state2 = (ClientHandshakePacketListenerImpl.State)this.state.updateAndGet((state2x) -> {
         if (!state.fromStates.contains(state2x)) {
            ...
            throw new IllegalStateException("Tried to switch to " + var10002 + " from " + String.valueOf(state2x) + ", but expected one of " + String.valueOf(state.fromStates));
         } else {
            return state;
         }
      });
   ...
   public void handleCustomQuery(ClientboundCustomQueryPacket clientboundCustomQueryPacket) {
      ...
      this.connection.send(new ServerboundCustomQueryAnswerPacket(clientboundCustomQueryPacket.transactionId(), (CustomQueryAnswerPayload)null));
   }
   ...
   public void handleRequestCookie(ClientboundCookieRequestPacket clientboundCookieRequestPacket) {
      this.connection.send(new ServerboundCookieResponsePacket(clientboundCookieRequestPacket.key(), (byte[])this.cookies.get(clientboundCookieRequestPacket.key())));
   }
