...
public class BubbleColumnAmbientSoundHandler implements AmbientSoundHandler {
   ...
   public void tick() {
      ...
      BlockState blockState = (BlockState)level.getBlockStatesIfLoaded(this.player.getBoundingBox().inflate(0.0D, -0.4000000059604645D, 0.0D).deflate(1.0E-6D)).filter((blockStatex) -> {
         return blockStatex.is(Blocks.BUBBLE_COLUMN);
