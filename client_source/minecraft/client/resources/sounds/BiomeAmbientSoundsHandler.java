...
public class BiomeAmbientSoundsHandler implements AmbientSoundHandler {
   ...
   public void tick() {
      ...
      if (biome != this.previousBiome) {
         ...
         biome.getAmbientLoop().ifPresent((holder) -> {
            this.loopSounds.compute(biome, (biomex, loopSoundInstance) -> {
      ...
      this.moodSettings.ifPresent((ambientMoodSettings) -> {
         ...
         if (this.moodiness >= 1.0F) {
            ...
            SimpleSoundInstance simpleSoundInstance = SimpleSoundInstance.forAmbientMood((SoundEvent)ambientMoodSettings.getSoundEvent().value(), this.random, this.player.getX() + g / l * m, this.player.getEyeY() + h / l * m, this.player.getZ() + k / l * m);
