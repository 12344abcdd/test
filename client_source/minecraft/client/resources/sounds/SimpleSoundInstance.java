...
public class SimpleSoundInstance extends AbstractSoundInstance {
   public SimpleSoundInstance(SoundEvent soundEvent, SoundSource soundSource, float f, float g, RandomSource randomSource, BlockPos blockPos) {
      this(soundEvent, soundSource, f, g, randomSource, (double)blockPos.getX() + 0.5D, (double)blockPos.getY() + 0.5D, (double)blockPos.getZ() + 0.5D);
   }
   ...
   public static SimpleSoundInstance forUI(SoundEvent soundEvent, float f, float g) {
      return new SimpleSoundInstance(soundEvent.getLocation(), SoundSource.MASTER, g, f, SoundInstance.createUnseededRandom(), false, 0, SoundInstance.Attenuation.NONE, 0.0D, 0.0D, 0.0D, true);
   }
   ...
   public static SimpleSoundInstance forLocalAmbience(SoundEvent soundEvent, float f, float g) {
      return new SimpleSoundInstance(soundEvent.getLocation(), SoundSource.AMBIENT, g, f, SoundInstance.createUnseededRandom(), false, 0, SoundInstance.Attenuation.NONE, 0.0D, 0.0D, 0.0D, true);
   }
