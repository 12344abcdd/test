...
public class DownloadedPackSource implements AutoCloseable {
   ...
   public DownloadedPackSource(Minecraft minecraft, Path path, GameConfig.UserData userData) {
      ...
      try {
         ...
         throw new UncheckedIOException("Failed to open download queue in directory " + String.valueOf(path), var5);
      }
   ...
   private PackDownloader createDownloader(DownloadQueue downloadQueue, Executor executor, User user, Proxy proxy) {
      return new PackDownloader() {
         ...
         private Map<String, String> createDownloadHeaders() {
            ...
            return Map.of("X-Minecraft-Username", user.getName(), "X-Minecraft-UUID", UndashedUuid.toString(user.getProfileId()), "X-Minecraft-Version", worldVersion.getName(), "X-Minecraft-Version-ID", worldVersion.getId(), "X-Minecraft-Pack-Format", String.valueOf(worldVersion.getPackVersion(PackType.CLIENT_RESOURCES)), "User-Agent", "Minecraft Java/" + worldVersion.getName());
         }

         public void download(Map<UUID, DownloadRequest> map, Consumer<BatchResult> consumer) {
            downloadQueue.downloadBatch(new BatchConfig(CACHE_HASHING_FUNCTION, 262144000, this.createDownloadHeaders(), proxy, DownloadedPackSource.this.createDownloadNotifier(map.size())), map).thenAcceptAsync(consumer, executor);
         }
      };
   }
   ...
   private List<Pack> loadRequestedPacks(List<PackReloadConfig.IdAndPath> list) {
      ...
      while(var3.hasNext()) {
         PackReloadConfig.IdAndPath idAndPath = (PackReloadConfig.IdAndPath)var3.next();
         ...
         Metadata metadata = Pack.readPackMetadata(packLocationInfo, resourcesSupplier, i);
         if (metadata == null) {
   ...
   private static PackLoadFeedback createPackResponseSender(Connection connection) {
      return new PackLoadFeedback() {
         public void reportUpdate(UUID uUID, PackLoadFeedback.Update update) {
            ...
            switch(update) {
            case ACCEPTED:
               var10000 = Action.ACCEPTED;
               break;
            case DOWNLOADED:
               var10000 = Action.DOWNLOADED;
               break;
            default:
               throw new MatchException((String)null, (Throwable)null);
            }

            Action action = var10000;
         ...
         public void reportFinalResult(UUID uUID, PackLoadFeedback.FinalResult finalResult) {
            ...
            switch(finalResult) {
            case APPLIED:
               var10000 = Action.SUCCESSFULLY_LOADED;
               break;
            case DOWNLOAD_FAILED:
               var10000 = Action.FAILED_DOWNLOAD;
               break;
            case DECLINED:
               var10000 = Action.DECLINED;
               break;
            case DISCARDED:
               var10000 = Action.DISCARDED;
               break;
            case ACTIVATION_FAILED:
               var10000 = Action.FAILED_RELOAD;
               break;
            default:
               throw new MatchException((String)null, (Throwable)null);
            }
   ...
   public CompletableFuture<Void> waitForPackFeedback(UUID uUID) {
      ...
      this.packFeedback = new PackLoadFeedback() {
         public void reportUpdate(UUID uUID, PackLoadFeedback.Update update) {
            packLoadFeedback.reportUpdate(uUIDx, update);
         }

         public void reportFinalResult(UUID uUID, PackLoadFeedback.FinalResult finalResult) {
            if (uUID.equals(uUIDx)) {
               ...
               if (finalResult == PackLoadFeedback.FinalResult.APPLIED) {
                  ...
                  String var10003 = String.valueOf(uUIDx);
                  var10000.completeExceptionally(new IllegalStateException("Failed to apply pack " + var10003 + ", reason: " + String.valueOf(finalResult)));
               }
            }

            packLoadFeedback.reportFinalResult(uUIDx, finalResult);
         }
      };
   ...
   static {
      DOWNLOADED_PACK_SELECTION = new PackSelectionConfig(true, Position.TOP, true);
