...
public class ModelManager implements PreparableReloadListener, AutoCloseable, FabricBakedModelManager {
   ...
   public final CompletableFuture<Void> reload(PreparationBarrier preparationBarrier, ResourceManager resourceManager, ProfilerFiller profilerFiller, ProfilerFiller profilerFiller2, Executor executor, Executor executor2) {
      ...
      CompletableFuture<ModelBakery> completableFuture3 = completableFuture.thenCombineAsync(completableFuture2, (mapx, map2) -> {
         return new ModelBakery(this.blockColors, profilerFiller, mapx, map2);
      }, executor);
      ...
      CompletableFuture var10000 = CompletableFuture.allOf((CompletableFuture[])Stream.concat(map.values().stream(), Stream.of(completableFuture3)).toArray((i) -> {
         ...
         return this.loadModels(profilerFiller, (Map)map.entrySet().stream().collect(Collectors.toMap(Entry::getKey, (entry) -> {
            return (AtlasSet.StitchResult)((CompletableFuture)entry.getValue()).join();
         })), (ModelBakery)completableFuture3.join());
      }, executor).thenCompose((reloadState) -> {
         return reloadState.readyForUpload.thenApply((void_) -> {
            return reloadState;
         });
      });
      Objects.requireNonNull(preparationBarrier);
      return var10000.thenCompose(preparationBarrier::wait).thenAcceptAsync((reloadState) -> {
         this.apply(reloadState, profilerFiller2);
      }, executor2);
   }
   ...
   private static CompletableFuture<Map<ResourceLocation, BlockModel>> loadBlockModels(ResourceManager resourceManager, Executor executor) {
      return CompletableFuture.supplyAsync(() -> {
         ...
         while(var3.hasNext()) {
            Entry<ResourceLocation, Resource> entry = (Entry)var3.next();
            list.add(CompletableFuture.supplyAsync(() -> {
               try {
                  ...
                  Pair var2;
                  try {
                     var2 = Pair.of((ResourceLocation)entry.getKey(), BlockModel.fromStream(reader));
         ...
         return Util.sequence(list).thenApply((listx) -> {
            return (Map)listx.stream().filter(Objects::nonNull).collect(Collectors.toUnmodifiableMap(Pair::getFirst, Pair::getSecond));
         });
      });
   }
   ...
   private static CompletableFuture<Map<ResourceLocation, List<BlockStateModelLoader.LoadedJson>>> loadBlockStates(ResourceManager resourceManager, Executor executor) {
      return CompletableFuture.supplyAsync(() -> {
         return BlockStateModelLoader.BLOCKSTATE_LISTER.listMatchingResourceStacks(resourceManager);
      }, executor).thenCompose((map) -> {
         List<CompletableFuture<Pair<ResourceLocation, List<BlockStateModelLoader.LoadedJson>>>> list = new ArrayList(map.size());
         Iterator var3 = map.entrySet().iterator();

         while(var3.hasNext()) {
            Entry<ResourceLocation, List<Resource>> entry = (Entry)var3.next();
            list.add(CompletableFuture.supplyAsync(() -> {
               List<Resource> list = (List)entry.getValue();
               List<BlockStateModelLoader.LoadedJson> list2 = new ArrayList(list.size());
               ...
               while(var3.hasNext()) {
                  ...
                  try {
                     ...
                     LOGGER.error("Failed to load blockstate {} from pack {}", new Object[]{entry.getKey(), resource.sourcePackId(), var10});
                  }
               }
   ...
   private ModelManager.ReloadState loadModels(ProfilerFiller profilerFiller, Map<ResourceLocation, AtlasSet.StitchResult> map, ModelBakery modelBakery) {
      ...
      multimap.asMap().forEach((modelResourceLocation, collection) -> {
         LOGGER.warn("Missing textures in model {}:\n{}", modelResourceLocation, collection.stream().sorted(Material.COMPARATOR).map((material) -> {
            String var10000 = String.valueOf(material.atlasLocation());
            return "    " + var10000 + ":" + String.valueOf(material.texture());
      ...
      CompletableFuture<Void> completableFuture = CompletableFuture.allOf((CompletableFuture[])map.values().stream().map(AtlasSet.StitchResult::readyForUpload).toArray((i) -> {
         return new CompletableFuture[i];
      }));
      profilerFiller.pop();
