...
public class FogRenderer {
   ...
   public static void setupColor(Camera camera, float f, ClientLevel clientLevel, int i, float g) {
      ...
      if (fogType == FogType.WATER) {
         ...
         Vec3 vec32 = camera.getPosition().subtract(2.0D, 2.0D, 2.0D).scale(0.25D);
   ...
   private static FogRenderer.MobEffectFogFunction getPriorityFogFunction(Entity entity, float f) {
      if (entity instanceof LivingEntity) {
         LivingEntity livingEntity = (LivingEntity)entity;
         return (FogRenderer.MobEffectFogFunction)MOB_EFFECT_FOG.stream().filter((mobEffectFogFunction) -> {
   ...
   @Environment(EnvType.CLIENT)
   private static class BlindnessFogFunction implements FogRenderer.MobEffectFogFunction {
