...
public class SectionOcclusionGraph {
   ...
   public void addSectionsInFrustum(Frustum frustum, List<SectionRenderDispatcher.RenderSection> list) {
      ...
      while(var3.hasNext()) {
         SectionOcclusionGraph.Node node = (SectionOcclusionGraph.Node)var3.next();
   ...
   private void initializeQueueForFullUpdate(Camera camera, Queue<SectionOcclusionGraph.Node> queue) {
      ...
      if (renderSection == null) {
         ...
         list.sort(Comparator.comparingDouble((nodex) -> {
            return blockPos.distSqr(nodex.section.getOrigin().offset(8, 8, 8));
         }));
         queue.addAll(list);
      } else {
         queue.add(new SectionOcclusionGraph.Node(renderSection, (Direction)null, 0));
      }

   }

   private void runUpdates(SectionOcclusionGraph.GraphStorage graphStorage, Vec3 vec3, Queue<SectionOcclusionGraph.Node> queue, boolean bl, Consumer<SectionRenderDispatcher.RenderSection> consumer) {
      int i = true;
      ...
      while(!queue.isEmpty()) {
         ...
         boolean bl2 = Math.abs(renderSection.getOrigin().getX() - blockPos.getX()) > 60 || Math.abs(renderSection.getOrigin().getY() - blockPos.getY()) > 60 || Math.abs(renderSection.getOrigin().getZ() - blockPos.getZ()) > 60;
         ...
         for(int var14 = 0; var14 < var13; ++var14) {
            ...
            if (renderSection2 != null && (!bl || !node.hasDirection(direction.getOpposite()))) {
               ...
               if (bl && bl2) {
                  ...
                  label105: {
                     SectionRenderDispatcher.RenderSection renderSection3;
                     do {
                        if (!(vec3.subtract(vec32).lengthSqr() > 3600.0D)) {
                           break label105;
                        }
                        ...
                        renderSection3 = this.viewArea.getRenderSectionAt(BlockPos.containing(vec32.x, vec32.y, vec32.z));
                     } while(renderSection3 != null && graphStorage.sectionToNodeMap.get(renderSection3) != null);
               ...
               if (node2 != null) {
                  ...
                  if (renderSection2.hasAllNeighbors()) {
                     ...
                     graphStorage.sectionToNodeMap.put(renderSection2, node3);
                     ((List)graphStorage.chunksWaitingForNeighbors.computeIfAbsent(ChunkPos.asLong(renderSection2.getOrigin()), (l) -> {
   ...
   private static record GraphState(SectionOcclusionGraph.GraphStorage storage, SectionOcclusionGraph.GraphEvents events) {
      ...
      private GraphState(SectionOcclusionGraph.GraphStorage graphStorage, SectionOcclusionGraph.GraphEvents graphEvents) {
