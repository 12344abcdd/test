...
public class EntityRenderDispatcher implements ResourceManagerReloadListener {
   ...
   public <E extends Entity> void render(E entity, double d, double e, double f, float g, float h, PoseStack poseStack, MultiBufferSource multiBufferSource, int i) {
      ...
      try {
         ...
         if ((Boolean)this.options.entityShadows().get() && this.shouldRenderShadow && !entity.isInvisible()) {
            float m = entityRenderer.getShadowRadius(entity);
            if (m > 0.0F) {
               double n = this.distanceToSqr(entity.getX(), entity.getY(), entity.getZ());
               float o = (float)((1.0D - n / 256.0D) * (double)entityRenderer.shadowStrength);
               if (o > 0.0F) {
   ...
   private static void renderServerSideHitbox(PoseStack poseStack, Entity entity, MultiBufferSource multiBufferSource) {
      ...
      if (entity2 == null) {
         DebugRenderer.renderFloatingText(poseStack, multiBufferSource, "Missing", entity.getX(), entity.getBoundingBox().maxY + 1.5D, entity.getZ(), -65536);
      } else {
         poseStack.pushPose();
   ...
   private static void renderHitbox(PoseStack poseStack, VertexConsumer vertexConsumer, Entity entity, float f, float g, float h, float i) {
      ...
      if (entity instanceof EnderDragon) {
         ...
         EnderDragonPart[] var14 = ((EnderDragon)entity).getSubEntities();
         int var15 = var14.length;

         for(int var16 = 0; var16 < var15; ++var16) {
            EnderDragonPart enderDragonPart = var14[var16];
            poseStack.pushPose();
      ...
      if (entity2 != null) {
         ...
         LevelRenderer.renderLineBox(poseStack, vertexConsumer, vec3.x - (double)o, vec3.y, vec3.z - (double)o, vec3.x + (double)o, vec3.y + 0.0625D, vec3.z + (double)o, 1.0F, 1.0F, 0.0F, 1.0F);
      }

      renderVector(poseStack, vertexConsumer, new Vector3f(0.0F, entity.getEyeHeight(), 0.0F), entity.getViewVector(f).scale(2.0D), -16776961);
   }
         ...
         if (blockState.isCollisionShapeFullBlock(chunkAccess, blockPos2)) {
            ...
            if (!voxelShape.isEmpty()) {
               ...
               if (j >= 0.0F) {
                  ...
                  int k = ARGB32.color(Mth.floor(j * 255.0F), 255, 255, 255);
