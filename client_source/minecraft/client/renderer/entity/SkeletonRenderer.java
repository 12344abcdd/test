...
public class SkeletonRenderer<T extends AbstractSkeleton> extends HumanoidMobRenderer<T, SkeletonModel<T>> {
   ...
   public SkeletonRenderer(EntityRendererProvider.Context context, ModelLayerLocation modelLayerLocation, ModelLayerLocation modelLayerLocation2, SkeletonModel<T> skeletonModel) {
      super(context, skeletonModel, 0.5F);
      this.addLayer(new HumanoidArmorLayer(this, new SkeletonModel(context.bakeLayer(modelLayerLocation)), new SkeletonModel(context.bakeLayer(modelLayerLocation2)), context.getModelManager()));
   }
   ...
   // $FF: synthetic method
   protected boolean isShaking(final LivingEntity livingEntity) {
