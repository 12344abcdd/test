...
public class EvokerRenderer<T extends SpellcasterIllager> extends IllagerRenderer<T> {
   ...
   public EvokerRenderer(EntityRendererProvider.Context context) {
      ...
      this.addLayer(new ItemInHandLayer<T, IllagerModel<T>>(this, this, context.getItemInHandRenderer()) {
         public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, T spellcasterIllager, float f, float g, float h, float j, float k, float l) {
            if (spellcasterIllager.isCastingSpell()) {
               super.render(poseStack, multiBufferSource, i, (LivingEntity)spellcasterIllager, f, g, h, j, k, l);
            }
