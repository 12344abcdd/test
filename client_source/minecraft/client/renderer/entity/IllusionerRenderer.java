...
public class IllusionerRenderer extends IllagerRenderer<Illusioner> {
   ...
   public IllusionerRenderer(EntityRendererProvider.Context context) {
      ...
      this.addLayer(new ItemInHandLayer<Illusioner, IllagerModel<Illusioner>>(this, this, context.getItemInHandRenderer()) {
         public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, Illusioner illusioner, float f, float g, float h, float j, float k, float l) {
            if (illusioner.isCastingSpell() || illusioner.isAggressive()) {
               super.render(poseStack, multiBufferSource, i, (LivingEntity)illusioner, f, g, h, j, k, l);
            }
   ...
   // $FF: synthetic method
   protected boolean isBodyVisible(final LivingEntity livingEntity) {
