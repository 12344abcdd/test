...
public class HorseMarkingLayer extends RenderLayer<Horse, HorseModel<Horse>> {
   private static final Map<Markings, ResourceLocation> LOCATION_BY_MARKINGS = (Map)Util.make(Maps.newEnumMap(Markings.class), (enumMap) -> {
      enumMap.put(Markings.NONE, (Object)null);
   ...
   public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, Horse horse, float f, float g, float h, float j, float k, float l) {
      ...
      if (resourceLocation != null && !horse.isInvisible()) {
         ...
         ((HorseModel)this.getParentModel()).renderToBuffer(poseStack, vertexConsumer, i, LivingEntityRenderer.getOverlayCoords(horse, 0.0F));
      }
   }
}
