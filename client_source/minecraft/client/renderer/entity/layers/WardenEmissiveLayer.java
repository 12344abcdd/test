...
public class WardenEmissiveLayer<T extends Warden, M extends WardenModel<T>> extends RenderLayer<T, M> {
   ...
   public WardenEmissiveLayer(RenderLayerParent<T, M> renderLayerParent, ResourceLocation resourceLocation, WardenEmissiveLayer.AlphaFunction<T> alphaFunction, WardenEmissiveLayer.DrawSelector<T, M> drawSelector) {
   ...
   public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, T warden, float f, float g, float h, float j, float k, float l) {
      if (!warden.isInvisible()) {
         ...
         int n = ARGB32.color(Mth.floor(m * 255.0F), 255, 255, 255);
         ((WardenModel)this.getParentModel()).renderToBuffer(poseStack, vertexConsumer, i, LivingEntityRenderer.getOverlayCoords(warden, 0.0F), n);
