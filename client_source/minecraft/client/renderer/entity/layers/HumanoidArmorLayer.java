...
public class HumanoidArmorLayer<T extends LivingEntity, M extends HumanoidModel<T>, A extends HumanoidModel<T>> extends RenderLayer<T, M> {
   ...
   private void renderArmorPiece(PoseStack poseStack, MultiBufferSource multiBufferSource, T livingEntity, EquipmentSlot equipmentSlot, int i, A humanoidModel) {
      ...
      if (var9 instanceof ArmorItem) {
         ...
         if (armorItem.getEquipmentSlot() == equipmentSlot) {
            ...
            int j = itemStack.is(ItemTags.DYEABLE) ? ARGB32.opaque(DyedItemColor.getOrDefault(itemStack, -6265536)) : -1;
            Iterator var12 = armorMaterial.layers().iterator();

            while(var12.hasNext()) {
               Layer layer = (Layer)var12.next();
   ...
   private void renderTrim(Holder<ArmorMaterial> holder, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, ArmorTrim armorTrim, A humanoidModel, boolean bl) {
