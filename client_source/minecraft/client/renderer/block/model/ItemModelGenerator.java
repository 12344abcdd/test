...
public class ItemModelGenerator {
   ...
   public BlockModel generateBlockModel(Function<Material, TextureAtlasSprite> function, BlockModel blockModel) {
      ...
      BlockModel blockModel2 = new BlockModel((ResourceLocation)null, list, map, false, blockModel.getGuiLight(), blockModel.getTransforms(), blockModel.getOverrides());
   ...
   private List<BlockElement> processFrames(int i, String string, SpriteContents spriteContents) {
      ...
      map.put(Direction.SOUTH, new BlockElementFace((Direction)null, i, string, new BlockFaceUV(new float[]{0.0F, 0.0F, 16.0F, 16.0F}, 0)));
      map.put(Direction.NORTH, new BlockElementFace((Direction)null, i, string, new BlockFaceUV(new float[]{16.0F, 0.0F, 0.0F, 16.0F}, 0)));
      List<BlockElement> list = Lists.newArrayList();
   ...
   private List<BlockElement> createSideElements(SpriteContents spriteContents, String string, int i) {
      ...
      List<BlockElement> list = Lists.newArrayList();
      Iterator var7 = this.getSpans(spriteContents).iterator();

      while(var7.hasNext()) {
         ItemModelGenerator.Span span = (ItemModelGenerator.Span)var7.next();
         float h = 0.0F;
         float j = 0.0F;
         float k = 0.0F;
         float l = 0.0F;
         ...
         p *= r;
         Map<Direction, BlockElementFace> map = Maps.newHashMap();
         map.put(spanFacing.getDirection(), new BlockElementFace((Direction)null, i, string, new BlockFaceUV(new float[]{m, o, n, p}, 0)));
         switch(spanFacing.ordinal()) {
         case 0:
            list.add(new BlockElement(new Vector3f(h, j, 7.5F), new Vector3f(k, j, 8.5F), map, (BlockElementRotation)null, true));
            break;
         case 1:
            list.add(new BlockElement(new Vector3f(h, l, 7.5F), new Vector3f(k, l, 8.5F), map, (BlockElementRotation)null, true));
            break;
         case 2:
            list.add(new BlockElement(new Vector3f(h, j, 7.5F), new Vector3f(h, l, 8.5F), map, (BlockElementRotation)null, true));
            break;
         case 3:
            list.add(new BlockElement(new Vector3f(k, j, 7.5F), new Vector3f(k, l, 8.5F), map, (BlockElementRotation)null, true));
         }
      }
   ...
   private void checkTransition(ItemModelGenerator.SpanFacing spanFacing, List<ItemModelGenerator.Span> list, SpriteContents spriteContents, int i, int j, int k, int l, int m, boolean bl) {
   ...
   private void createOrExpandSpan(List<ItemModelGenerator.Span> list, ItemModelGenerator.SpanFacing spanFacing, int i, int j) {
      ...
      while(var6.hasNext()) {
         ItemModelGenerator.Span span2 = (ItemModelGenerator.Span)var6.next();
