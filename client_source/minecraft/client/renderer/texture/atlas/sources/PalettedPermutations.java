...
public class PalettedPermutations implements SpriteSource {
   ...
   public static final MapCodec<PalettedPermutations> CODEC = RecordCodecBuilder.mapCodec((instance) -> {
      ...
      while(true) {
         while(var5.hasNext()) {
            ...
            if (optional.isEmpty()) {
               ...
               while(var10.hasNext()) {
                  Entry<String, Supplier<IntUnaryOperator>> entry = (Entry)var10.next();
                  ResourceLocation resourceLocation3 = resourceLocation.withSuffix("_" + (String)entry.getKey());
                  output.add(resourceLocation3, (SpriteSource.SpriteSupplier)(new PalettedPermutations.PalettedSpriteSupplier(lazyLoadedImage, (Supplier)entry.getValue(), resourceLocation3)));
               }
            }
         }
   ...
   private static IntUnaryOperator createPaletteMapping(int[] is, int[] js) {
      if (js.length != is.length) {
         ...
         for(int i = 0; i < is.length; ++i) {
            int j = is[i];
            if (ABGR32.alpha(j) != 0) {
               int2IntMap.put(ABGR32.transparent(j), js[i]);
            }
         }

         return (ix) -> {
            int j = ABGR32.alpha(ix);
            if (j == 0) {
               return ix;
            } else {
               int k = ABGR32.transparent(ix);
               int l = int2IntMap.getOrDefault(k, ABGR32.opaque(k));
               int m = ABGR32.alpha(l);
               return ABGR32.color(j * m / 255, l);
            }
         };
      }
   }
   ...
   static record PalettedSpriteSupplier(LazyLoadedImage baseImage, Supplier<IntUnaryOperator> palette, ResourceLocation permutationLocation) implements SpriteSource.SpriteSupplier {
      ...
      public SpriteContents apply(SpriteResourceLoader spriteResourceLoader) {
         ...
         try {
            ...
            var3 = new SpriteContents(this.permutationLocation, new FrameSize(nativeImage.getWidth(), nativeImage.getHeight()), nativeImage, ResourceMetadata.EMPTY);
            return var3;
