...
public class VaultRenderer implements BlockEntityRenderer<VaultBlockEntity> {
   ...
   public void render(VaultBlockEntity vaultBlockEntity, float f, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, int j) {
      if (Client.shouldDisplayActiveEffects(vaultBlockEntity.getSharedData())) {
         ...
         if (level != null) {
            ...
            if (!itemStack.isEmpty()) {
               ...
               renderItemInside(f, level, poseStack, multiBufferSource, i, itemStack, this.itemRenderer, vaultClientData.previousSpin(), vaultClientData.currentSpin(), this.random);
            }
         }
      }
   }

   public static void renderItemInside(float f, Level level, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, ItemStack itemStack, ItemRenderer itemRenderer, float g, float h, RandomSource randomSource) {
      poseStack.pushPose();
