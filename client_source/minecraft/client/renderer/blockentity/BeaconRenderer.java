...
public class BeaconRenderer implements BlockEntityRenderer<BeaconBlockEntity> {
   ...
   public void render(BeaconBlockEntity beaconBlockEntity, float f, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, int j) {
      ...
      List<BeaconBeamSection> list = beaconBlockEntity.getBeamSections();
      int k = 0;

      for(int m = 0; m < list.size(); ++m) {
         BeaconBeamSection beaconBeamSection = (BeaconBeamSection)list.get(m);
   ...
   public static void renderBeaconBeam(PoseStack poseStack, MultiBufferSource multiBufferSource, ResourceLocation resourceLocation, float f, float g, long l, int i, int j, int k, float h, float m) {
      int n = i + j;
      poseStack.pushPose();
      ...
      renderPart(poseStack, multiBufferSource.getBuffer(RenderType.beaconBeam(resourceLocation, false)), k, i, n, 0.0F, h, h, 0.0F, v, 0.0F, 0.0F, y, 0.0F, 1.0F, ac, ab);
      poseStack.popPose();
      r = -m;
      float s = -m;
      u = -m;
      ...
      ab = -1.0F + q;
      ac = (float)j * g + ab;
      renderPart(poseStack, multiBufferSource.getBuffer(RenderType.beaconBeam(resourceLocation, true)), ARGB32.color(32, k), i, n, r, s, m, u, v, m, m, m, 0.0F, 1.0F, ac, ab);
      poseStack.popPose();
   }

   private static void renderPart(PoseStack poseStack, VertexConsumer vertexConsumer, int i, int j, int k, float f, float g, float h, float l, float m, float n, float o, float p, float q, float r, float s, float t) {
   ...
   private static void addVertex(PoseStack.Pose pose, VertexConsumer vertexConsumer, int i, int j, float f, float g, float h, float k) {
      vertexConsumer.addVertex(pose, f, (float)j, g).setColor(i).setUv(h, k).setOverlay(OverlayTexture.NO_OVERLAY).setLight(15728880).setNormal(pose, 0.0F, 1.0F, 0.0F);
   }
