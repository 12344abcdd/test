...
public class ConduitRenderer implements BlockEntityRenderer<ConduitBlockEntity> {
   ...
   public void render(ConduitBlockEntity conduitBlockEntity, float f, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, int j) {
      ...
      if (!conduitBlockEntity.isActive()) {
         h = conduitBlockEntity.getActiveRotation(0.0F);
         VertexConsumer vertexConsumer = SHELL_TEXTURE.buffer(multiBufferSource, RenderType::entitySolid);
         poseStack.pushPose();
         poseStack.translate(0.5F, 0.5F, 0.5F);
         poseStack.mulPose((new Quaternionf()).rotationY(h * 0.017453292F));
         this.shell.render(poseStack, vertexConsumer, i, j);
         poseStack.popPose();
      } else {
         h = conduitBlockEntity.getActiveRotation(f) * 57.295776F;
         ...
         poseStack.mulPose((new Quaternionf()).rotationAxis(h * 0.017453292F, vector3f));
         ...
         poseStack.mulPose((new Quaternionf()).rotationYXZ(m * 0.017453292F, camera.getXRot() * 0.017453292F, 3.1415927F));
         float n = 1.3333334F;
