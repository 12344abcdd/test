...
public class BrainDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
   ...
   private void renderPoiInfo(PoseStack poseStack, MultiBufferSource multiBufferSource, BrainDebugRenderer.PoiInfo poiInfo) {
      ...
      if (set.size() < 4) {
         renderTextOverPoi(poseStack, multiBufferSource, "Owners: " + String.valueOf(set), poiInfo, i, -256);
      } else {
      ...
      if (set2.size() < 4) {
         renderTextOverPoi(poseStack, multiBufferSource, "Candidates: " + String.valueOf(set2), poiInfo, i, -23296);
      } else {
   ...
   private void renderBrainInfo(PoseStack poseStack, MultiBufferSource multiBufferSource, BrainDump brainDump, double d, double e, double f) {
      ...
      if (bl) {
         ...
         Vec3 var10002 = brainDump.pos();
         String var10004 = String.format(Locale.ROOT, "%.1f", brainDump.health());
         renderTextOverMob(poseStack, multiBufferSource, var10002, i, "health: " + var10004 + " / " + String.format(Locale.ROOT, "%.1f", brainDump.maxHealth()), j, 0.02F);
         ++i;
      }
      ...
      Iterator var15;
      if (bl) {
         for(var15 = brainDump.behaviors().iterator(); var15.hasNext(); ++i) {
            string = (String)var15.next();
            renderTextOverMob(poseStack, multiBufferSource, brainDump.pos(), i, string, -16711681, 0.02F);
         }
      }

      if (bl) {
         for(var15 = brainDump.activities().iterator(); var15.hasNext(); ++i) {
            string = (String)var15.next();
      ...
      if (bl) {
         for(var15 = brainDump.gossips().iterator(); var15.hasNext(); ++i) {
            string = (String)var15.next();
      ...
      if (bl) {
         for(var15 = Lists.reverse(brainDump.memories()).iterator(); var15.hasNext(); ++i) {
            string = (String)var15.next();
   ...
   private static void renderTextOverPoi(PoseStack poseStack, MultiBufferSource multiBufferSource, String string, BrainDebugRenderer.PoiInfo poiInfo, int i, int j) {
   ...
   private Set<String> getTicketHolderNames(BrainDebugRenderer.PoiInfo poiInfo) {
      return (Set)this.getTicketHolders(poiInfo.pos).stream().map(DebugEntityNameGenerator::getEntityName).collect(Collectors.toSet());
   }

   private Set<String> getPotentialTicketHolderNames(BrainDebugRenderer.PoiInfo poiInfo) {
      return (Set)this.getPotentialTicketHolders(poiInfo.pos).stream().map(DebugEntityNameGenerator::getEntityName).collect(Collectors.toSet());
   }
   ...
   private Collection<UUID> getTicketHolders(BlockPos blockPos) {
      ...
      }).map(BrainDump::uuid).collect(Collectors.toSet());
   }
   ...
   private Map<BlockPos, List<String>> getGhostPois() {
      ...
      while(var2.hasNext()) {
         BrainDump brainDump = (BrainDump)var2.next();
         Iterator var4 = Iterables.concat(brainDump.pois(), brainDump.potentialPois()).iterator();
