...
public class DebugRenderer {
   ...
   public static Optional<Entity> getTargetedEntity(@Nullable Entity entity, int i) {
      if (entity == null) {
         ...
         AABB aABB = entity.getBoundingBox().expandTowards(vec32).inflate(1.0D);
         int j = i * i;
   ...
   public static void renderFloatingText(PoseStack poseStack, MultiBufferSource multiBufferSource, String string, int i, int j, int k, int l) {
      renderFloatingText(poseStack, multiBufferSource, string, (double)i + 0.5D, (double)j + 0.5D, (double)k + 0.5D, l);
   }
   ...
   public static void renderFloatingText(PoseStack poseStack, MultiBufferSource multiBufferSource, String string, double d, double e, double f, int i, float g, boolean bl, float h, boolean bl2) {
      ...
      if (camera.isInitialized() && minecraft.getEntityRenderDispatcher().options != null) {
         ...
         m -= h / g;
         font.drawInBatch((String)string, m, 0.0F, i, false, poseStack.last().pose(), multiBufferSource, bl2 ? Font.DisplayMode.SEE_THROUGH : Font.DisplayMode.NORMAL, 0, 15728880);
         poseStack.popPose();
      }
   }
