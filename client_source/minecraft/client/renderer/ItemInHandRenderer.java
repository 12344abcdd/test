...
public class ItemInHandRenderer {
   ...
   private float calculateMapTilt(float f) {
      ...
      g = -Mth.cos(g * 3.1415927F) * 0.5F + 0.5F;
      return g;
   }
   ...
   private void renderOneHandedMap(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, float f, HumanoidArm humanoidArm, float g, ItemStack itemStack) {
      ...
      float j = Mth.sqrt(g);
      float k = Mth.sin(j * 3.1415927F);
      float l = -0.5F * k;
      float m = 0.4F * Mth.sin(j * 6.2831855F);
      float n = -0.3F * Mth.sin(g * 3.1415927F);
   ...
   private void renderTwoHandedMap(PoseStack poseStack, MultiBufferSource multiBufferSource, int i, float f, float g, float h) {
      ...
      float l = -0.4F * Mth.sin(j * 3.1415927F);
   ...
   private void applyEatTransform(PoseStack poseStack, float f, HumanoidArm humanoidArm, ItemStack itemStack, Player player) {
      ...
      float i;
      if (h < 0.8F) {
         i = Mth.abs(Mth.cos(g / 4.0F * 3.1415927F) * 0.1F);
         poseStack.translate(0.0F, i, 0.0F);
      }

      i = 1.0F - (float)Math.pow((double)h, 27.0D);
   ...
   private void applyBrushTransform(PoseStack poseStack, float f, HumanoidArm humanoidArm, ItemStack itemStack, Player player, float g) {
      ...
      float n = -15.0F;
      int o = true;
      float p = -15.0F + 75.0F * Mth.cos(j * 2.0F * 3.1415927F);
      if (humanoidArm != HumanoidArm.RIGHT) {
         ...
         if (itemStack.isEmpty()) {
            ...
            if (itemStack.is(Items.CROSSBOW)) {
               ...
               if (abstractClientPlayer.isUsingItem() && abstractClientPlayer.getUseItemRemainingTicks() > 0 && abstractClientPlayer.getUsedItemHand() == interactionHand) {
                  ...
                  l = (float)itemStack.getUseDuration(abstractClientPlayer) - ((float)abstractClientPlayer.getUseItemRemainingTicks() - f + 1.0F);
                  m = l / (float)CrossbowItem.getChargeDuration(itemStack, abstractClientPlayer);
                  if (m > 1.0F) {
                     m = 1.0F;
                  }
               ...
               bl2 = humanoidArm == HumanoidArm.RIGHT;
               int q;
               float r;
               if (abstractClientPlayer.isUsingItem() && abstractClientPlayer.getUseItemRemainingTicks() > 0 && abstractClientPlayer.getUsedItemHand() == interactionHand) {
                  q = bl2 ? 1 : -1;
                  ...
                  case BOW:
                     ...
                     r = (float)itemStack.getUseDuration(abstractClientPlayer) - ((float)abstractClientPlayer.getUseItemRemainingTicks() - f + 1.0F);
                     l = r / 20.0F;
                     l = (l * l + l * 2.0F) / 3.0F;
                     if (l > 1.0F) {
                        l = 1.0F;
                  ...
                  float s = -0.4F * Mth.sin(Mth.sqrt(h) * 3.1415927F);
                  r = 0.2F * Mth.sin(Mth.sqrt(h) * 6.2831855F);
                  l = -0.2F * Mth.sin(h * 3.1415927F);
                  int t = bl2 ? 1 : -1;
