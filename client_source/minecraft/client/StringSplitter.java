...
public class StringSplitter {
   ...
   public Style componentStyleAtWidth(FormattedText formattedText, int i) {
      StringSplitter.WidthLimitedCharSink widthLimitedCharSink = new StringSplitter.WidthLimitedCharSink((float)i);
      return (Style)formattedText.visit((style, string) -> {
         return StringDecomposer.iterateFormatted(string, style, widthLimitedCharSink) ? Optional.empty() : Optional.of(style);
   ...
   public FormattedText headByWidth(FormattedText formattedText, int i, Style style) {
      ...
      return (FormattedText)formattedText.visit(new StyledContentConsumer<FormattedText>(this) {
   ...
   public List<FormattedText> splitLines(String string, int i, Style style) {
      ...
      this.splitLines(string, i, style, false, (stylex, ix, j) -> {
         list.add(FormattedText.of(string.substring(ix, j), stylex));
      });
      return list;
   }
   ...
   public List<FormattedText> splitLines(FormattedText formattedText, int i, Style style) {
      List<FormattedText> list = Lists.newArrayList();
      this.splitLines(formattedText, i, style, (formattedTextx, boolean_) -> {
         list.add(formattedTextx);
      });
      return list;
   }
   ...
   public List<FormattedText> splitLines(FormattedText formattedText, int i, Style style, FormattedText formattedText2) {
      List<FormattedText> list = Lists.newArrayList();
      this.splitLines(formattedText, i, style, (formattedText2x, boolean_) -> {
         list.add(boolean_ ? FormattedText.composite(new FormattedText[]{formattedText2, formattedText2x}) : formattedText2x);
      });
      return list;
   }
   ...
   public void splitLines(FormattedText formattedText, int i, Style style, BiConsumer<FormattedText, Boolean> biConsumer) {
      ...
      while(true) {
         while(bl) {
            bl = false;
            StringSplitter.LineBreakFinder lineBreakFinder = new StringSplitter.LineBreakFinder((float)i);
            Iterator var11 = flatComponents.parts.iterator();

            while(var11.hasNext()) {
               StringSplitter.LineComponent lineComponent = (StringSplitter.LineComponent)var11.next();
   ...
   static class FlatComponents {
      ...
      public FormattedText getRemainder() {
         ...
         var10000.forEach(componentCollector::append);
         this.parts.clear();
