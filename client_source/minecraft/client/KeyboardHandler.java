...
public class KeyboardHandler {
   ...
   private void debugComponent(ChatFormatting chatFormatting, Component component) {
      this.minecraft.gui.getChat().addMessage(Component.empty().append(Component.translatable("debug.prefix").withStyle(new ChatFormatting[]{chatFormatting, ChatFormatting.BOLD})).append(CommonComponents.SPACE).append(component));
   }
      ...
      if (this.debugCrashKeyTime > 0L && this.debugCrashKeyTime < Util.getMillis() - 100L) {
         ...
         case 83:
            ...
            Component component = Component.literal(path.relativize(path2).toString()).withStyle(ChatFormatting.UNDERLINE).withStyle((style) -> {
               return style.withClickEvent(new ClickEvent(Action.OPEN_FILE, path2.toFile().toString()));
            });
   ...
   public void keyPress(long l, int i, int j, int k, int m) {
      if (l == this.minecraft.getWindow().getWindow()) {
         ...
         if (k != 0) {
            ...
            if (bl2) {
               if (Screen.hasControlDown() && i == 66 && this.minecraft.getNarrator().isActive() && (Boolean)this.minecraft.options.narratorHotkey().get()) {
                  bl3 = this.minecraft.options.narrator().get() == NarratorStatus.OFF;
      ...
      if (l == this.minecraft.getWindow().getWindow()) {
         ...
         if (guiEventListener != null && this.minecraft.getOverlay() == null) {
            if (Character.charCount(i) == 1) {
               Screen.wrapScreenError(() -> {
                  guiEventListener.charTyped((char)i, j);
               ...
               for(int var8 = 0; var8 < var7; ++var8) {
                  char c = var6[var8];
                  Screen.wrapScreenError(() -> {
                     guiEventListener.charTyped(c, j);
                  }, "charTyped event handler", guiEventListener.getClass().getCanonicalName());
               }
            }
   ...
   public void setup(long l) {
      InputConstants.setupKeyboardCallbacks(l, (lx, i, j, k, m) -> {
         this.minecraft.execute(() -> {
            this.keyPress(lx, i, j, k, m);
         });
      }, (lx, i, j) -> {
         this.minecraft.execute(() -> {
            this.charTyped(lx, i, j);
         });
      });
   }
