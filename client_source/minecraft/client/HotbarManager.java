...
public class HotbarManager {
   ...
   private void load() {
      try {
         ...
         for(int j = 0; j < 9; ++j) {
            this.hotbars[j] = (Hotbar)Hotbar.CODEC.parse(NbtOps.INSTANCE, compoundTag.get(String.valueOf(j))).resultOrPartial((string) -> {
               LOGGER.warn("Failed to parse hotbar: {}", string);
