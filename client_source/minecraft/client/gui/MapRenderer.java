...
public class MapRenderer implements AutoCloseable {
   ...
   private MapRenderer.MapInstance getOrCreateMapInstance(MapId mapId, MapItemSavedData mapItemSavedData) {
      return (MapRenderer.MapInstance)this.maps.compute(mapId.id(), (integer, mapInstance) -> {
         if (mapInstance == null) {
   ...
   public void resetData() {
      ...
      while(var1.hasNext()) {
         MapRenderer.MapInstance mapInstance = (MapRenderer.MapInstance)var1.next();
         mapInstance.close();
      }
   ...
   private class MapInstance implements AutoCloseable {
      ...
      void draw(PoseStack poseStack, MultiBufferSource multiBufferSource, boolean bl, int i) {
         ...
         while(true) {
            ...
            } while(bl && !mapDecoration.renderOnFrame());
            ...
            if (mapDecoration.name().isPresent()) {
               ...
               float p = (float)font.width((FormattedText)component);
               ...
               font.drawInBatch((Component)component, 0.0F, 0.0F, -1, false, poseStack.last().pose(), multiBufferSource, Font.DisplayMode.NORMAL, Integer.MIN_VALUE, i);
               poseStack.popPose();
            }
