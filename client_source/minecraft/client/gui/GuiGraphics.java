...
public class GuiGraphics {
   ...
   public int drawString(Font font, FormattedCharSequence formattedCharSequence, int i, int j, int k, boolean bl) {
      int l = font.drawInBatch((FormattedCharSequence)formattedCharSequence, (float)i, (float)j, k, bl, this.pose.last().pose(), this.bufferSource, Font.DisplayMode.NORMAL, 0, 15728880);
      this.flushIfUnmanaged();
      return l;
   }
   ...
   public void drawWordWrap(Font font, FormattedText formattedText, int i, int j, int k, int l) {
      for(Iterator var7 = font.split(formattedText, k).iterator(); var7.hasNext(); j += 9) {
   ...
   public int drawStringWithBackdrop(Font font, Component component, int i, int j, int k, int l) {
      ...
      if (m != 0) {
         ...
         int var10003 = i + k + 2;
         Objects.requireNonNull(font);
         this.fill(var10001, var10002, var10003, j + 9 + 2, ARGB32.multiply(m, l));
      }
   ...
   public void blit(int i, int j, int k, int l, int m, TextureAtlasSprite textureAtlasSprite, float f, float g, float h, float n) {
      this.innerBlit(textureAtlasSprite.atlasLocation(), i, i + l, j, j + m, k, textureAtlasSprite.getU0(), textureAtlasSprite.getU1(), textureAtlasSprite.getV0(), textureAtlasSprite.getV1(), f, g, h, n);
   }
   ...
   public void blitSprite(ResourceLocation resourceLocation, int i, int j, int k, int l) {
      this.blitSprite((ResourceLocation)resourceLocation, i, j, 0, k, l);
   }
   ...
   public void blitSprite(ResourceLocation resourceLocation, int i, int j, int k, int l, int m, int n, int o, int p) {
      this.blitSprite((ResourceLocation)resourceLocation, i, j, k, l, m, n, 0, o, p);
   }
   ...
   private void blitSprite(TextureAtlasSprite textureAtlasSprite, int i, int j, int k, int l, int m, int n, int o, int p, int q) {
      if (p != 0 && q != 0) {
         this.innerBlit(textureAtlasSprite.atlasLocation(), m, m + p, n, n + q, o, textureAtlasSprite.getU((float)k / (float)i), textureAtlasSprite.getU((float)(k + p) / (float)i), textureAtlasSprite.getV((float)l / (float)j), textureAtlasSprite.getV((float)(l + q) / (float)j));
      }
   }
   ...
   private void blitNineSlicedSprite(TextureAtlasSprite textureAtlasSprite, GuiSpriteScaling.NineSlice nineSlice, int i, int j, int k, int l, int m) {
      ...
      if (l == nineSlice.width() && m == nineSlice.height()) {
         this.blitSprite((TextureAtlasSprite)textureAtlasSprite, nineSlice.width(), nineSlice.height(), 0, 0, i, j, k, l, m);
      } else if (m == nineSlice.height()) {
         this.blitSprite((TextureAtlasSprite)textureAtlasSprite, nineSlice.width(), nineSlice.height(), 0, 0, i, j, k, n, m);
         ...
         this.blitSprite((TextureAtlasSprite)textureAtlasSprite, nineSlice.width(), nineSlice.height(), nineSlice.width() - o, 0, i + l - o, j, k, o, m);
      } else if (l == nineSlice.width()) {
         this.blitSprite((TextureAtlasSprite)textureAtlasSprite, nineSlice.width(), nineSlice.height(), 0, 0, i, j, k, l, p);
         ...
         this.blitSprite((TextureAtlasSprite)textureAtlasSprite, nineSlice.width(), nineSlice.height(), 0, nineSlice.height() - q, i, j + m - q, k, l, q);
      } else {
         this.blitSprite((TextureAtlasSprite)textureAtlasSprite, nineSlice.width(), nineSlice.height(), 0, 0, i, j, k, n, p);
         this.blitTiledSprite(textureAtlasSprite, i + n, j, k, l - o - n, p, n, 0, nineSlice.width() - o - n, p, nineSlice.width(), nineSlice.height());
         this.blitSprite((TextureAtlasSprite)textureAtlasSprite, nineSlice.width(), nineSlice.height(), nineSlice.width() - o, 0, i + l - o, j, k, o, p);
         this.blitSprite((TextureAtlasSprite)textureAtlasSprite, nineSlice.width(), nineSlice.height(), 0, nineSlice.height() - q, i, j + m - q, k, n, q);
   ...
   public void renderFakeItem(ItemStack itemStack, int i, int j, int k) {
      this.renderItem((LivingEntity)null, this.minecraft.level, itemStack, i, j, k);
   }
   ...
   public void renderItemDecorations(Font font, ItemStack itemStack, int i, int j, @Nullable String string) {
      if (!itemStack.isEmpty()) {
         ...
         if (itemStack.isBarVisible()) {
            ...
            this.fill(RenderType.guiOverlay(), m, n, m + k, n + 1, l | -16777216);
         }
   ...
   public void renderTooltip(Font font, List<Component> list, Optional<TooltipComponent> optional, int i, int j) {
      ...
      optional.ifPresent((tooltipComponent) -> {
         list2.add(list2.isEmpty() ? 0 : 1, ClientTooltipComponent.create(tooltipComponent));
      });
   ...
   public void renderTooltip(Font font, List<? extends FormattedCharSequence> list, int i, int j) {
      this.renderTooltipInternal(font, (List)list.stream().map(ClientTooltipComponent::create).collect(Collectors.toList()), i, j, DefaultTooltipPositioner.INSTANCE);
   }
   ...
   public void renderTooltip(Font font, List<FormattedCharSequence> list, ClientTooltipPositioner clientTooltipPositioner, int i, int j) {
      this.renderTooltipInternal(font, (List)list.stream().map(ClientTooltipComponent::create).collect(Collectors.toList()), i, j, clientTooltipPositioner);
   }
   ...
   private void renderTooltipInternal(Font font, List<ClientTooltipComponent> list, int i, int j, ClientTooltipPositioner clientTooltipPositioner) {
      if (!list.isEmpty()) {
         ...
         for(Iterator var8 = list.iterator(); var8.hasNext(); l += clientTooltipComponent.getHeight()) {
         ...
         for(t = 0; t < list.size(); ++t) {
            clientTooltipComponent2 = (ClientTooltipComponent)list.get(t);
