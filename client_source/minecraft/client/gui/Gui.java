...
public class Gui {
   ...
   public Gui(Minecraft minecraft) {
      ...
      LayeredDraw layeredDraw = (new LayeredDraw()).add(this::renderCameraOverlays).add(this::renderCrosshair).add(this::renderHotbarAndDecorations).add(this::renderExperienceLevel).add(this::renderEffects).add((guiGraphics, deltaTracker) -> {
         this.bossOverlay.render(guiGraphics);
      });
      LayeredDraw layeredDraw2 = (new LayeredDraw()).add(this::renderDemoOverlay).add((guiGraphics, deltaTracker) -> {
         if (this.debugOverlay.showDebugScreen()) {
            this.debugOverlay.render(guiGraphics);
         }

      }).add(this::renderScoreboardSidebar).add(this::renderOverlayMessage).add(this::renderTitle).add(this::renderChat).add(this::renderTabList).add((guiGraphics, deltaTracker) -> {
         this.subtitleOverlay.render(guiGraphics);
      });
      this.layers.add(layeredDraw, () -> {
         return !minecraft.options.hideGui;
      }).add(this::renderSleepOverlay).add(layeredDraw2, () -> {
         return !minecraft.options.hideGui;
      });
   }
   ...
   private void renderCameraOverlays(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
      ...
      float g = Mth.lerp(deltaTracker.getGameTimeDeltaPartialTick(false), this.minecraft.player.oSpinningEffectIntensity, this.minecraft.player.spinningEffectIntensity);
   ...
   private void renderOverlayMessage(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
      ...
      if (this.overlayMessageString != null && this.overlayMessageTime > 0) {
         ...
         if (i > 8) {
            ...
            int k = font.width((FormattedText)this.overlayMessageString);
   ...
   private void renderTitle(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
      if (this.title != null && this.titleTime > 0) {
         ...
         if (i > 8) {
            ...
            int j = font.width((FormattedText)this.title);
            int k = ARGB32.color(i, -1);
            guiGraphics.drawStringWithBackdrop(font, this.title, -j / 2, -10, j, k);
            guiGraphics.pose().popPose();
            if (this.subtitle != null) {
               guiGraphics.pose().pushPose();
               guiGraphics.pose().scale(2.0F, 2.0F, 2.0F);
               int l = font.width((FormattedText)this.subtitle);
   ...
   private void renderCrosshair(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
      ...
      if (options.getCameraType().isFirstPerson()) {
         if (this.minecraft.gameMode.getPlayerMode() != GameType.SPECTATOR || this.canRenderCrosshairForSpectator(this.minecraft.hitResult)) {
            ...
            if (this.debugOverlay.showDebugScreen() && !this.minecraft.player.isReducedDebugInfo() && !(Boolean)options.reducedDebugInfo().get()) {
               ...
               matrix4fStack.rotateX(-camera.getXRot() * 0.017453292F);
               matrix4fStack.rotateY(camera.getYRot() * 0.017453292F);
   ...
   private void renderEffects(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
      ...
      label40: {
         collection = this.minecraft.player.getActiveEffects();
   ...
   private void renderItemHotbar(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
      ...
      if (player != null) {
         ...
         for(m = 0; m < 9; ++m) {
            ...
            this.renderSlot(guiGraphics, n, o, deltaTracker, player, (ItemStack)player.getInventory().items.get(m), l++);
         }
   ...
   private void renderExperienceLevel(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
      ...
      if (this.isExperienceBarVisible() && i > 0) {
         ...
         String string = i.makeConcatWithConstants<invokedynamic>(i);
         int j = (guiGraphics.guiWidth() - this.getFont().width(string)) / 2;
         int k = guiGraphics.guiHeight() - 31 - 4;
         guiGraphics.drawString(this.getFont(), (String)string, j + 1, k, 0, false);
         guiGraphics.drawString(this.getFont(), (String)string, j - 1, k, 0, false);
         guiGraphics.drawString(this.getFont(), (String)string, j, k + 1, 0, false);
         guiGraphics.drawString(this.getFont(), (String)string, j, k - 1, 0, false);
   ...
   private void renderSelectedItemName(GuiGraphics guiGraphics) {
      ...
      if (this.toolHighlightTimer > 0 && !this.lastToolHighlight.isEmpty()) {
         ...
         int i = this.getFont().width((FormattedText)mutableComponent);
         ...
         if (l > 0) {
            guiGraphics.drawStringWithBackdrop(this.getFont(), mutableComponent, j, k, i, ARGB32.color(l, -1));
         }
      }
   ...
   private void renderDemoOverlay(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
      if (this.minecraft.isDemo()) {
         ...
         if (this.minecraft.level.getGameTime() >= 120500L) {
            ...
            component = Component.translatable("demo.remainingTime", new Object[]{StringUtil.formatTickDuration((int)(120500L - this.minecraft.level.getGameTime()), this.minecraft.level.tickRateManager().tickrate())});
         }

         int i = this.getFont().width((FormattedText)component);
         int j = guiGraphics.guiWidth() - i - 10;
         int k = true;
         guiGraphics.drawStringWithBackdrop(this.getFont(), (Component)component, j, 5, i, -1);
         this.minecraft.getProfiler().pop();
      }
   }
   ...
   private void displayScoreboardSidebar(GuiGraphics guiGraphics, Objective objective) {
      ...
      DisplayEntry[] lvs = (DisplayEntry[])scoreboard.listPlayerScores(objective).stream().filter((playerScoreEntry) -> {
         return !playerScoreEntry.isHidden();
      }).sorted(SCORE_DISPLAY_ORDER).limit(15L).map((playerScoreEntry) -> {
         PlayerTeam playerTeam = scoreboard.getPlayersTeam(playerScoreEntry.owner());
         Component component = playerScoreEntry.ownerName();
         Component component2 = PlayerTeam.formatNameForTeam(playerTeam, component);
         Component component3 = playerScoreEntry.formatValue(numberFormat);
         int i = this.getFont().width((FormattedText)component3);
         ...
         return new DisplayEntry[ix];
      });
      Component component = objective.getDisplayName();
      int i = this.getFont().width((FormattedText)component);
      int j = i;
      int k = this.getFont().width(": ");
      DisplayEntry[] var10 = lvs;
      int var11 = lvs.length;

      for(int var12 = 0; var12 < var11; ++var12) {
         DisplayEntry lv = var10[var12];
         j = Math.max(j, this.getFont().width((FormattedText)lv.name) + (lv.scoreWidth > 0 ? k + lv.scoreWidth : 0));
      }
      ...
      guiGraphics.drawManaged(() -> {
         ...
         int var10001 = o - 2;
         Objects.requireNonNull(this.getFont());
         guiGraphics.fill(var10001, s - 9 - 1, p, s - 1, r);
         guiGraphics.fill(o - 2, s - 1, p, m, q);
         Font var18 = this.getFont();
         int var10003 = o + j / 2 - i / 2;
         Objects.requireNonNull(this.getFont());
         guiGraphics.drawString(var18, (Component)component, var10003, s - 9, -1, false);

         for(int t = 0; t < k; ++t) {
            DisplayEntry lv = lvs[t];
            var10001 = k - t;
            Objects.requireNonNull(this.getFont());
            int u = m - var10001 * 9;
            guiGraphics.drawString(this.getFont(), (Component)lv.name, o, u, -1, false);
            guiGraphics.drawString(this.getFont(), (Component)lv.score, p - lv.scoreWidth, u, -1, false);
         }
   ...
   private void renderPlayerHealth(GuiGraphics guiGraphics) {
      ...
      if (player != null) {
         ...
         if (player.isEyeInFluid(FluidTags.WATER) || v < u) {
            ...
            r -= w * 10;
            int x = Mth.ceil((double)(v - 2) * 10.0D / (double)u);
            int y = Mth.ceil((double)v * 10.0D / (double)u) - x;
            RenderSystem.enableBlend();
   ...
   private void renderHearts(GuiGraphics guiGraphics, Player player, int i, int j, int k, int l, float f, int m, int n, int o, boolean bl) {
      ...
      int p = Mth.ceil((double)f / 2.0D);
      int q = Mth.ceil((double)o / 2.0D);
      int r = p * 2;
   ...
   private void renderVignette(GuiGraphics guiGraphics, @Nullable Entity entity) {
      ...
      float g;
      if (entity != null) {
         g = (float)worldBorder.getDistanceToBorder(entity);
         double d = Math.min(worldBorder.getLerpSpeed() * (double)worldBorder.getWarningTime() * 1000.0D, Math.abs(worldBorder.getLerpTarget() - worldBorder.getSize()));
   ...
   public void setNowPlaying(Component component) {
      ...
      this.minecraft.getNarrator().sayNow((Component)component2);
   }
   ...
   public void renderSavingIndicator(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
      if ((Boolean)this.minecraft.options.showAutosaveIndicator().get() && (this.autosaveIndicatorValue > 0.0F || this.lastAutosaveIndicatorValue > 0.0F)) {
         int i = Mth.floor(255.0F * Mth.clamp(Mth.lerp(deltaTracker.getRealtimeDeltaTicks(), this.lastAutosaveIndicatorValue, this.autosaveIndicatorValue), 0.0F, 1.0F));
         if (i > 8) {
            Font font = this.getFont();
            int j = font.width((FormattedText)SAVING_TEXT);
            int k = ARGB32.color(i, -1);
            int l = guiGraphics.guiWidth() - j - 2;
