...
public class FontTexture extends AbstractTexture implements Dumpable {
   ...
   public BakedGlyph add(SheetGlyphInfo sheetGlyphInfo) {
      if (sheetGlyphInfo.isColored() != this.colored) {
         ...
         if (node != null) {
            ...
            float g = 256.0F;
            float h = 0.01F;
            return new BakedGlyph(this.renderTypes, ((float)node.x + 0.01F) / 256.0F, ((float)node.x - 0.01F + (float)sheetGlyphInfo.getPixelWidth()) / 256.0F, ((float)node.y + 0.01F) / 256.0F, ((float)node.y - 0.01F + (float)sheetGlyphInfo.getPixelHeight()) / 256.0F, sheetGlyphInfo.getLeft(), sheetGlyphInfo.getRight(), sheetGlyphInfo.getTop(), sheetGlyphInfo.getBottom());
         } else {
            return null;
         }
      }
   ...
   public void dumpContents(ResourceLocation resourceLocation, Path path) {
      ...
      TextureUtil.writeAsPNG(path, string, this.getId(), 0, 256, 256, (i) -> {
         return (i & -16777216) == 0 ? -16777216 : i;
      });
   }
