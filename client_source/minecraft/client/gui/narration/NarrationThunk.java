...
public class NarrationThunk<T> {
   ...
   public static NarrationThunk<?> from(Component component) {
      return new NarrationThunk(component, (consumer, componentx) -> {
         consumer.accept(componentx.getString());
      });
   }

   public static NarrationThunk<?> from(List<Component> list) {
      return new NarrationThunk(list, (consumer, list2) -> {
         list.stream().map(Component::getString).forEach(consumer);
      });
   }
   ...
   static {
      EMPTY = new NarrationThunk(Unit.INSTANCE, (consumer, unit) -> {
      });
   }
}
