...
public class ServerSelectionList extends ObjectSelectionList<ServerSelectionList.Entry> {
   ...
   private void refreshEntries() {
      ...
      this.networkServers.forEach((entry) -> {
         this.addEntry(entry);
      });
   }
   ...
   public boolean keyPressed(int i, int j, int k) {
      ServerSelectionList.Entry entry = (ServerSelectionList.Entry)this.getSelected();
   ...
   public void updateNetworkServers(List<LanServer> list) {
      ...
      while(var3.hasNext()) {
         LanServer lanServer = (LanServer)var3.next();
      ...
      for(int j = this.networkServers.size() - i; j < this.networkServers.size(); ++j) {
         ...
         if (m >= this.getY() && l <= this.getBottom()) {
            this.minecraft.getNarrator().say(Component.translatable("multiplayer.lan.server_found", new Object[]{networkServerEntry.getServerNarration()}));
         }
      }
   ...
   public static class LANHeader extends ServerSelectionList.Entry {
      ...
      public void render(GuiGraphics guiGraphics, int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
         ...
         int p = var10000 - 9 / 2;
         guiGraphics.drawString(this.minecraft.font, ServerSelectionList.SCANNING_LABEL, this.minecraft.screen.width / 2 - this.minecraft.font.width((FormattedText)ServerSelectionList.SCANNING_LABEL) / 2, p, 16777215, false);
         String string = LoadingDotsText.get(Util.getMillis());
         Font var10001 = this.minecraft.font;
         int var10003 = this.minecraft.screen.width / 2 - this.minecraft.font.width(string) / 2;
         Objects.requireNonNull(this.minecraft.font);
         guiGraphics.drawString(var10001, string, var10003, p + 9, -8355712, false);
      }
   ...
   public class OnlineServerEntry extends ServerSelectionList.Entry {
      ...
      public void render(GuiGraphics guiGraphics, int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
         ...
         for(p = 0; p < Math.min(list.size(), 2); ++p) {
            ...
            int var10004 = j + 12;
            Objects.requireNonNull(this.minecraft.font);
            guiGraphics.drawString(var10001, var10002, var10003, var10004 + 9 * p, -8355712, false);
         }

         this.drawIcon(guiGraphics, k, j, this.icon.textureLocation());
         if (this.serverData.state() == ServerData.State.PINGING) {
            p = (int)(Util.getMillis() / 100L + (long)(i * 2) & 7L);
            if (p > 4) {
               p = 8 - p;
            }
         ...
         int r = p - q - 5;
         guiGraphics.drawString(this.minecraft.font, (Component)component, r, j + 1, -8355712, false);
         if (this.statusIconTooltip != null && n >= p && n <= p + 10 && o >= j && o <= j + 8) {
            this.screen.setTooltipForNextRenderPass(this.statusIconTooltip);
         } else if (this.onlinePlayersTooltip != null && n >= r && n <= r + q && o >= j) {
            int var20 = j - 1;
            Objects.requireNonNull(this.minecraft.font);
            if (o <= var20 + 9) {
      ...
      public boolean mouseClicked(double d, double e, int i) {
         ...
         if (f <= 32.0D) {
            ...
            if (f < 16.0D && g < 16.0D && j > 0) {
               this.swap(j, j - 1);
               return true;
            }

            if (f < 16.0D && g > 16.0D && j < this.screen.getServers().size() - 1) {
               this.swap(j, j + 1);
               return true;
            }
         }
