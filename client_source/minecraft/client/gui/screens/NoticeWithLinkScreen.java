...
public class NoticeWithLinkScreen extends Screen {
   ...
   protected void init() {
      ...
      rowHelper2.addChild(Button.builder(CommonComponents.GUI_OPEN_IN_BROWSER, (button) -> {
         Util.getPlatform().openUri(this.uri);
      }).size(120, 20).build());
      rowHelper2.addChild(Button.builder(CommonComponents.GUI_COPY_LINK_TO_CLIPBOARD, (button) -> {
         this.minecraft.keyboardHandler.setClipboard(this.uri.toString());
      }).size(120, 20).build());
      rowHelper2.addChild(Button.builder(CommonComponents.GUI_BACK, (button) -> {
         this.onClose();
      }).size(120, 20).build());
   ...
   static {
      ...
      SYMLINK_WORLD_MESSAGE_TEXT = Component.translatable("symlink_warning.message.world", new Object[]{Component.translationArg(CommonLinks.SYMLINK_HELP)});
      SYMLINK_PACK_TITLE = Component.translatable("symlink_warning.title.pack").withStyle(ChatFormatting.BOLD);
      SYMLINK_PACK_MESSAGE_TEXT = Component.translatable("symlink_warning.message.pack", new Object[]{Component.translationArg(CommonLinks.SYMLINK_HELP)});
   }
}
