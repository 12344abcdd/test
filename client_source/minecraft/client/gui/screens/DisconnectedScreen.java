...
public class DisconnectedScreen extends Screen {
   ...
   protected void init() {
      ...
      this.details.bugReportLink().ifPresent((uRI) -> {
         this.layout.addChild(Button.builder(REPORT_TO_SERVER_TITLE, ConfirmLinkScreen.confirmLink(this, (URI)uRI, false)).width(200).build());
      });
      this.details.report().ifPresent((path) -> {
         this.layout.addChild(Button.builder(OPEN_REPORT_DIR_TITLE, (button) -> {
            Util.getPlatform().openPath(path.getParent());
         }).width(200).build());
      });
      Button button;
      if (this.minecraft.allowsMultiplayer()) {
         button = Button.builder(this.buttonText, (buttonx) -> {
            this.minecraft.setScreen(this.parent);
         }).width(200).build();
      } else {
         button = Button.builder(TO_TITLE, (buttonx) -> {
