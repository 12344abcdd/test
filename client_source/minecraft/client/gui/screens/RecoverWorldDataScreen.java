...
public class RecoverWorldDataScreen extends Screen {
   ...
   public RecoverWorldDataScreen(Minecraft minecraft, BooleanConsumer booleanConsumer, LevelStorageAccess levelStorageAccess) {
      ...
      this.message = Component.translatable("recover_world.message", new Object[]{Component.literal(levelStorageAccess.getLevelId()).withStyle(ChatFormatting.GRAY)});
      ...
      Component component = Component.empty().append(this.buildInfo(levelStorageAccess, false, exception)).append("\n").append(this.buildInfo(levelStorageAccess, true, exception2));
      ...
      linearLayout.addChild(Button.builder(BUGTRACKER_BUTTON, ConfirmLinkScreen.confirmLink(this, (URI)CommonLinks.SNAPSHOT_BUGS_FEEDBACK)).size(120, 20).build());
      ((Button)linearLayout.addChild(Button.builder(RESTORE_BUTTON, (button) -> {
         this.attemptRestore(minecraft);
      }).size(120, 20).tooltip(bl ? null : Tooltip.create(NO_FALLBACK_TOOLTIP)).build())).active = bl;
      this.layout.addChild(linearLayout);
      this.layout.addChild(Button.builder(CommonComponents.GUI_BACK, (button) -> {
         this.onClose();
      }).size(120, 20).build());
