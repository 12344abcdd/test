...
public class SocialInteractionsScreen extends Screen {
   ...
   public Component getNarrationMessage() {
      return (Component)(this.serverLabel != null ? CommonComponents.joinForNarration(new Component[]{super.getNarrationMessage(), this.serverLabel}) : super.getNarrationMessage());
   }
   ...
   protected void init() {
      ...
      this.allButton = (Button)this.addRenderableWidget(Button.builder(TAB_ALL, (button) -> {
         this.showPage(SocialInteractionsScreen.Page.ALL);
      }).bounds(j, 45, i, 20).build());
      this.hiddenButton = (Button)this.addRenderableWidget(Button.builder(TAB_HIDDEN, (button) -> {
         this.showPage(SocialInteractionsScreen.Page.HIDDEN);
      }).bounds((j + k - i) / 2 + 1, 45, i, 20).build());
      this.blockedButton = (Button)this.addRenderableWidget(Button.builder(TAB_BLOCKED, (button) -> {
   ...
   private void showPage(SocialInteractionsScreen.Page page) {
      ...
      case 2:
         ...
         Set<UUID> set2 = (Set)var10000.filter(playerSocialManager::isBlocked).collect(Collectors.toSet());
         bl = set2.isEmpty();
   ...
   public void render(GuiGraphics guiGraphics, int i, int j, float f) {
      ...
      if (this.serverLabel != null) {
         guiGraphics.drawString(this.minecraft.font, (Component)this.serverLabel, this.marginX() + 8, 35, -1);
      }

      if (!this.socialInteractionsPlayerList.isEmpty()) {
         this.socialInteractionsPlayerList.render(guiGraphics, i, j, f);
      } else if (!this.searchBox.getValue().isEmpty()) {
         guiGraphics.drawCenteredString(this.minecraft.font, (Component)EMPTY_SEARCH, this.width / 2, (72 + this.listEnd()) / 2, -1);
      } else if (this.page == SocialInteractionsScreen.Page.HIDDEN) {
         guiGraphics.drawCenteredString(this.minecraft.font, (Component)EMPTY_HIDDEN, this.width / 2, (72 + this.listEnd()) / 2, -1);
      } else if (this.page == SocialInteractionsScreen.Page.BLOCKED) {
         guiGraphics.drawCenteredString(this.minecraft.font, (Component)EMPTY_BLOCKED, this.width / 2, (72 + this.listEnd()) / 2, -1);
      }
