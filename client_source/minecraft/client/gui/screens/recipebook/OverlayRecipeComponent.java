...
public class OverlayRecipeComponent implements Renderable, GuiEventListener {
   ...
   public void init(Minecraft minecraft, RecipeCollection recipeCollection, int i, int j, int k, int l, float f) {
      ...
      for(int u = 0; u < n; ++u) {
         ...
         if (this.isFurnaceMenu) {
            this.recipeButtons.add(new OverlayRecipeComponent.OverlaySmeltingRecipeButton(this, v, w, recipeHolder, bl2));
         } else {
   ...
   public boolean mouseClicked(double d, double e, int i) {
      if (i != 0) {
         ...
         OverlayRecipeComponent.OverlayRecipeButton overlayRecipeButton;
         do {
            if (!var6.hasNext()) {
               return false;
            }

            overlayRecipeButton = (OverlayRecipeComponent.OverlayRecipeButton)var6.next();
         } while(!overlayRecipeButton.mouseClicked(d, e, i));
   ...
   public void render(GuiGraphics guiGraphics, int i, int j, float f) {
      if (this.isVisible) {
         ...
         while(var9.hasNext()) {
            OverlayRecipeComponent.OverlayRecipeButton overlayRecipeButton = (OverlayRecipeComponent.OverlayRecipeButton)var9.next();
   ...
   private class OverlaySmeltingRecipeButton extends OverlayRecipeComponent.OverlayRecipeButton {
      public OverlaySmeltingRecipeButton(final OverlayRecipeComponent overlayRecipeComponent, final int i, final int j, final RecipeHolder recipeHolder, final boolean bl) {
   ...
   private class OverlayRecipeButton extends AbstractWidget implements PlaceRecipe<Ingredient> {
      ...
      public void addItemToSlot(Ingredient ingredient, int i, int j, int k, int l) {
         ...
         if (itemStacks.length != 0) {
            this.ingredientPos.add(new OverlayRecipeComponent.OverlayRecipeButton.Pos(this, 3 + k * 7, 3 + l * 7, itemStacks));
         }
      ...
      public void renderWidget(GuiGraphics guiGraphics, int i, int j, float f) {
         ...
         for(Iterator var6 = this.ingredientPos.iterator(); var6.hasNext(); guiGraphics.pose().popPose()) {
            OverlayRecipeComponent.OverlayRecipeButton.Pos pos = (OverlayRecipeComponent.OverlayRecipeButton.Pos)var6.next();
            guiGraphics.pose().pushPose();
            guiGraphics.pose().translate((double)pos.x, (double)pos.y, 0.0D);
      ...
      protected class Pos {
         ...
         public Pos(final OverlayRecipeComponent.OverlayRecipeButton overlayRecipeButton, final int i, final int j, final ItemStack[] itemStacks) {
            this.x = i;
            this.y = j;
