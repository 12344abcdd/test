...
public abstract class Screen extends AbstractContainerEventHandler implements Renderable {
   ...
   public boolean keyPressed(int i, int j, int k) {
      if (i == 256 && this.shouldCloseOnEsc()) {
         ...
         FocusNavigationEvent focusNavigationEvent = var10000;
   ...
   protected void setInitialFocus(GuiEventListener guiEventListener) {
      ComponentPath componentPath = ComponentPath.path((ContainerEventHandler)this, (ComponentPath)guiEventListener.nextFocusPath(new FocusNavigationEvent.InitialFocus()));
      if (componentPath != null) {
         this.changeFocus(componentPath);
      }
   ...
   public static List<Component> getTooltipFromItem(Minecraft minecraft, ItemStack itemStack) {
      return itemStack.getTooltipLines(TooltipContext.of(minecraft.level), minecraft.player, minecraft.options.advancedItemTooltips ? Default.ADVANCED : Default.NORMAL);
   }
   ...
   protected void updateNarratedWidget(NarrationElementOutput narrationElementOutput) {
      List<NarratableEntry> list = this.narratables.stream().filter(NarratableEntry::isActive).sorted(Comparator.comparingInt(TabOrderedElement::getTabOrderGroup)).toList();
      ...
      if (narratableSearchResult != null) {
         ...
         if (list.size() > 1) {
            narrationElementOutput.add(NarratedElementType.POSITION, (Component)Component.translatable("narrator.position.screen", new Object[]{narratableSearchResult.index + 1, list.size()}));
