...
public class PackSelectionModel {
   ...
   public Stream<PackSelectionModel.Entry> getUnselected() {
      return this.unselected.stream().map((pack) -> {
         return new PackSelectionModel.UnselectedPackEntry(pack);
      });
   }

   public Stream<PackSelectionModel.Entry> getSelected() {
      return this.selected.stream().map((pack) -> {
         return new PackSelectionModel.SelectedPackEntry(pack);
      });
   }
   ...
   private abstract class EntryBase implements PackSelectionModel.Entry {
      ...
      private void updateHighContrastOptionInstance() {
         if (this.pack.getId().equals("high_contrast")) {
            ...
            optionInstance.set(!(Boolean)optionInstance.get());
         }
