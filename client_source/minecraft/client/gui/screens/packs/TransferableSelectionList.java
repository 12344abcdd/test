...
public class TransferableSelectionList extends ObjectSelectionList<TransferableSelectionList.PackEntry> {
   ...
   protected void renderHeader(GuiGraphics guiGraphics, int i, int j) {
      ...
      guiGraphics.drawString(this.minecraft.font, (Component)component, i + this.width / 2 - this.minecraft.font.width((FormattedText)component) / 2, Math.min(this.getY() + 3, j), -1, false);
   }
   ...
   public static class PackEntry extends ObjectSelectionList.Entry<TransferableSelectionList.PackEntry> {
      ...
      private static FormattedCharSequence cacheName(Minecraft minecraft, Component component) {
         ...
         if (i > 157) {
            FormattedText formattedText = FormattedText.composite(new FormattedText[]{minecraft.font.substrByWidth(component, 157 - minecraft.font.width("...")), FormattedText.of("...")});
      ...
      public boolean mouseClicked(double d, double e, int i) {
         ...
         if (this.showHoverOverlay() && f <= 32.0D) {
            ...
            if (f < 16.0D && this.pack.canUnselect()) {
               this.pack.unselect();
               return true;
            }

            if (f > 16.0D && g < 16.0D && this.pack.canMoveUp()) {
               this.pack.moveUp();
               return true;
            }

            if (f > 16.0D && g > 16.0D && this.pack.canMoveDown()) {
               this.pack.moveDown();
               return true;
            }
         }
