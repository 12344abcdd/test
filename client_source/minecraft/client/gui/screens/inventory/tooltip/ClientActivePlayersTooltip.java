...
public class ClientActivePlayersTooltip implements ClientTooltipComponent {
   ...
   public void renderImage(Font font, int i, int j, GuiGraphics guiGraphics) {
      for(int k = 0; k < this.activePlayers.size(); ++k) {
         ...
         int l = j + 2 + k * 12;
         PlayerFaceRenderer.draw(guiGraphics, (PlayerSkin)Minecraft.getInstance().getSkinManager().getInsecureSkin(profileResult.profile()), i + 2, l, 10);
         guiGraphics.drawString(font, (String)profileResult.profile().getName(), i + 10 + 4, l + 2, -1);
      }
