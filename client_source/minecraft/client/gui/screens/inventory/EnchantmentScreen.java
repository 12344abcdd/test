...
public class EnchantmentScreen extends AbstractContainerScreen<EnchantmentMenu> {
   ...
   public boolean mouseClicked(double d, double e, int i) {
      ...
      for(int l = 0; l < 3; ++l) {
         ...
         if (f >= 0.0D && g >= 0.0D && f < 108.0D && g < 19.0D && ((EnchantmentMenu)this.menu).clickMenuButton(this.minecraft.player, l)) {
   ...
   protected void renderBg(GuiGraphics guiGraphics, float f, int i, int j) {
      ...
      for(int n = 0; n < 3; ++n) {
         ...
         if (q == 0) {
            ...
            String string = q.makeConcatWithConstants<invokedynamic>(q);
   ...
   public void render(GuiGraphics guiGraphics, int i, int j, float f) {
      ...
      for(int l = 0; l < 3; ++l) {
         ...
         Optional<Reference<Enchantment>> optional = this.minecraft.level.registryAccess().registryOrThrow(Registries.ENCHANTMENT).getHolder(((EnchantmentMenu)this.menu).enchantClue[l]);
         if (!optional.isEmpty()) {
            ...
            if (this.isHovering(60, 14 + 19 * l, 108, 17, (double)i, (double)j) && m > 0 && n >= 0 && optional != null) {
               ...
               list.add(Component.translatable("container.enchant.clue", new Object[]{Enchantment.getFullname((Holder)optional.get(), n)}).withStyle(ChatFormatting.WHITE));
               if (!bl) {
                  list.add(CommonComponents.EMPTY);
                  if (this.minecraft.player.experienceLevel < m) {
                     list.add(Component.translatable("container.enchant.level.requirement", new Object[]{((EnchantmentMenu)this.menu).costs[l]}).withStyle(ChatFormatting.RED));
                  } else {
