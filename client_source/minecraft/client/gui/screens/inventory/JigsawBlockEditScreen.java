...
public class JigsawBlockEditScreen extends Screen {
   ...
   protected void init() {
      ...
      this.poolEdit.setResponder((string) -> {
         this.updateValidity();
      });
      ...
      this.targetEdit.setResponder((string) -> {
         this.updateValidity();
      });
      ...
      this.jointButton = (CycleButton)this.addRenderableWidget(CycleButton.builder(JointType::getTranslatedName).withValues((Object[])JointType.values()).withInitialValue(this.joint).displayOnlyValue().create(this.width / 2 + 54, 160, 100, 20, JOINT_LABEL, (cycleButton, jointType) -> {
   ...
   private void updateValidity() {
      boolean bl = isValidResourceLocation(this.nameEdit.getValue()) && isValidResourceLocation(this.targetEdit.getValue()) && isValidResourceLocation(this.poolEdit.getValue());
   ...
   public void render(GuiGraphics guiGraphics, int i, int j, float f) {
      ...
      guiGraphics.drawString(this.font, (Component)POOL_LABEL, this.width / 2 - 153, 10, 10526880);
      this.poolEdit.render(guiGraphics, i, j, f);
      guiGraphics.drawString(this.font, (Component)NAME_LABEL, this.width / 2 - 153, 45, 10526880);
      this.nameEdit.render(guiGraphics, i, j, f);
      guiGraphics.drawString(this.font, (Component)TARGET_LABEL, this.width / 2 - 153, 80, 10526880);
      this.targetEdit.render(guiGraphics, i, j, f);
      guiGraphics.drawString(this.font, (Component)FINAL_STATE_LABEL, this.width / 2 - 153, 115, 10526880);
      this.finalStateEdit.render(guiGraphics, i, j, f);
      guiGraphics.drawString(this.font, (Component)SELECTION_PRIORITY_LABEL, this.width / 2 - 153, 150, 10526880);
      this.placementPriorityEdit.render(guiGraphics, i, j, f);
      guiGraphics.drawString(this.font, (Component)PLACEMENT_PRIORITY_LABEL, this.width / 2 - 50, 150, 10526880);
      ...
      if (JigsawBlock.getFrontFacing(this.jigsawEntity.getBlockState()).getAxis().isVertical()) {
         guiGraphics.drawString(this.font, (Component)JOINT_LABEL, this.width / 2 + 53, 150, 10526880);
      }
