...
public class BeaconScreen extends AbstractContainerScreen<BeaconMenu> {
   ...
   private <T extends AbstractWidget & BeaconScreen.BeaconButton> void addBeaconButton(T abstractWidget) {
      ...
      this.beaconButtons.add((BeaconScreen.BeaconButton)abstractWidget);
   }
   ...
   protected void init() {
      ...
      for(int i = 0; i <= 2; ++i) {
         j = ((List)BeaconBlockEntity.BEACON_EFFECTS.get(i)).size();
         k = j * 22 + (j - 1) * 2;

         for(l = 0; l < j; ++l) {
            holder = (Holder)((List)BeaconBlockEntity.BEACON_EFFECTS.get(i)).get(l);
            beaconPowerButton = new BeaconScreen.BeaconPowerButton(this.leftPos + 76 + l * 24 - k / 2, this.topPos + 22 + i * 25, holder, true, i);
      ...
      int i = true;
      j = ((List)BeaconBlockEntity.BEACON_EFFECTS.get(3)).size() + 1;
      k = j * 22 + (j - 1) * 2;

      for(l = 0; l < j - 1; ++l) {
         holder = (Holder)((List)BeaconBlockEntity.BEACON_EFFECTS.get(3)).get(l);
         beaconPowerButton = new BeaconScreen.BeaconPowerButton(this.leftPos + 167 + l * 24 - k / 2, this.topPos + 47, holder, false, 3);
         beaconPowerButton.active = false;
         this.addBeaconButton(beaconPowerButton);
      }

      Holder<MobEffect> holder2 = (Holder)((List)BeaconBlockEntity.BEACON_EFFECTS.get(0)).get(0);
      BeaconScreen.BeaconPowerButton beaconPowerButton2 = new BeaconScreen.BeaconUpgradePowerButton(this.leftPos + 167 + (j - 1) * 24 - k / 2, this.topPos + 47, holder2);
   ...
   void updateButtons() {
      ...
      this.beaconButtons.forEach((beaconButton) -> {
         beaconButton.updateStatus(i);
      });
   }

   protected void renderLabels(GuiGraphics guiGraphics, int i, int j) {
      guiGraphics.drawCenteredString(this.font, (Component)PRIMARY_EFFECT_LABEL, 62, 10, 14737632);
      guiGraphics.drawCenteredString(this.font, (Component)SECONDARY_EFFECT_LABEL, 169, 10, 14737632);
   }
   ...
   private class BeaconPowerButton extends BeaconScreen.BeaconScreenButton {
      ...
      protected void setEffect(Holder<MobEffect> holder) {
         ...
         this.setTooltip(Tooltip.create(this.createEffectDescription(holder), (Component)null));
      }
