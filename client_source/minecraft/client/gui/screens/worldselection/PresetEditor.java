...
public interface PresetEditor {
   Map<Optional<ResourceKey<WorldPreset>>, PresetEditor> EDITORS = Map.of(Optional.of(WorldPresets.FLAT), (createWorldScreen, worldCreationContext) -> {
      ...
      return new CreateFlatWorldScreen(createWorldScreen, (flatLevelGeneratorSettings) -> {
         createWorldScreen.getUiState().updateDimensions(flatWorldConfigurator(flatLevelGeneratorSettings));
      }, chunkGenerator instanceof FlatLevelSource ? ((FlatLevelSource)chunkGenerator).settings() : FlatLevelGeneratorSettings.getDefault(holderGetter, holderGetter2, holderGetter3));
   }, Optional.of(WorldPresets.SINGLE_BIOME_SURFACE), (createWorldScreen, worldCreationContext) -> {
      return new CreateBuffetWorldScreen(createWorldScreen, worldCreationContext, (holder) -> {
         createWorldScreen.getUiState().updateDimensions(fixedBiomeConfigurator(holder));
      });
   });
