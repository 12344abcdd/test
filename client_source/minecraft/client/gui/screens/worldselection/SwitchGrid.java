...
class SwitchGrid {
   ...
   public static class Builder {
      ...
      public SwitchGrid build(Consumer<LayoutElement> consumer) {
         ...
         while(var4.hasNext()) {
            SwitchGrid.SwitchBuilder switchBuilder = (SwitchGrid.SwitchBuilder)var4.next();
   ...
   public static class SwitchBuilder {
      ...
      SwitchGrid.LabeledSwitch build(SwitchGrid.Builder builder, GridLayout gridLayout, int i) {
         ...
         gridLayout.addChild(stringWidget, builder.rowCount, i, (LayoutSettings)gridLayout.newCellSettings().align(0.0F, 0.5F).paddingLeft(builder.paddingLeft));
         ...
         if (bl) {
            ...
            builder2.withTooltip((boolean_) -> {
               return tooltip;
            });
         }

         if (this.info != null && !bl) {
            builder2.withCustomNarration((cycleButtonx) -> {
               return CommonComponents.joinForNarration(new Component[]{this.label, cycleButtonx.createDefaultNarrationMessage(), this.info});
            });
         } else {
            builder2.withCustomNarration((cycleButtonx) -> {
               return CommonComponents.joinForNarration(new Component[]{this.label, cycleButtonx.createDefaultNarrationMessage()});
            });
         }

         CycleButton<Boolean> cycleButton = builder2.create(0, 0, this.buttonWidth, 20, Component.empty(), (cycleButtonx, boolean_) -> {
            this.onClicked.accept(boolean_);
         });
         if (this.isActiveCondition != null) {
            cycleButton.active = this.isActiveCondition.getAsBoolean();
         }

         gridLayout.addChild(cycleButton, builder.rowCount, i + 1, (LayoutSettings)gridLayout.newCellSettings().alignHorizontallyRight());
         if (this.info != null) {
            optional.ifPresent((infoUnderneathSettings) -> {
               ...
               if (infoUnderneathSettings.alwaysMaxHeight) {
                  ...
                  var10000 = 9 * infoUnderneathSettings.maxInfoRows - multiLineTextWidget.getHeight();
               } else {
                  var10000 = 0;
               }

               int j = var10000;
               gridLayout.addChild(multiLineTextWidget, builder.rowCount, i, (LayoutSettings)gridLayout.newCellSettings().paddingTop(-builder.rowSpacing).paddingBottom(j));
            });
         }
