...
public class CycleButton<T> extends AbstractButton {
   ...
   CycleButton(int i, int j, int k, int l, Component component, Component component2, int m, T object, CycleButton.ValueListSupplier<T> valueListSupplier, Function<T, Component> function, Function<CycleButton<T>, MutableComponent> function2, CycleButton.OnValueChange<T> onValueChange, OptionInstance.TooltipSupplier<T> tooltipSupplier, boolean bl) {
   ...
   public void updateWidgetNarration(NarrationElementOutput narrationElementOutput) {
      ...
      if (this.active) {
         ...
         Component component = this.createLabelForValue(object);
         if (this.isFocused()) {
            narrationElementOutput.add(NarratedElementType.USAGE, (Component)Component.translatable("narration.cycle_button.usage.focused", new Object[]{component}));
         } else {
            narrationElementOutput.add(NarratedElementType.USAGE, (Component)Component.translatable("narration.cycle_button.usage.hovered", new Object[]{component}));
         }
      }
   ...
   public static CycleButton.Builder<Boolean> booleanBuilder(Component component, Component component2) {
      return (new CycleButton.Builder((boolean_) -> {
         return boolean_ ? component : component2;
   ...
   public interface ValueListSupplier<T> {
      ...
      static <T> CycleButton.ValueListSupplier<T> create(Collection<T> collection) {
         List<T> list = ImmutableList.copyOf(collection);
      ...
      static <T> CycleButton.ValueListSupplier<T> create(BooleanSupplier booleanSupplier, List<T> list, List<T> list2) {
         List<T> list3 = ImmutableList.copyOf(list);
         List<T> list4 = ImmutableList.copyOf(list2);
         return new CycleButton.ValueListSupplier<T>(list4, list3) {
            // $FF: synthetic field
            ...
            {
               this.val$altSelector = booleanSupplier;
   ...
   public static class Builder<T> {
      ...
      private final Function<T, Component> valueStringifier;
      private OptionInstance.TooltipSupplier<T> tooltipSupplier = (object) -> {
         return null;
      };
      ...
      public final CycleButton.Builder<T> withValues(T... objects) {
         return this.withValues((Collection)ImmutableList.copyOf(objects));
      }
      ...
      public CycleButton<T> create(int i, int j, int k, int l, Component component, CycleButton.OnValueChange<T> onValueChange) {
         ...
         if (list.isEmpty()) {
            ...
            return new CycleButton(i, j, k, l, (Component)component3, component, this.initialIndex, object, this.values, this.valueStringifier, this.narrationProvider, onValueChange, this.tooltipSupplier, this.displayOnlyValue);
         }
      }
   }
}
