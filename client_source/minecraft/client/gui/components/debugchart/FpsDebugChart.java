...
public class FpsDebugChart extends AbstractDebugChart {
   ...
   protected void renderAdditionalLinesAndLabels(GuiGraphics guiGraphics, int i, int j, int k) {
      ...
      int l = (Integer)Minecraft.getInstance().options.framerateLimit().get();
      if (l > 0 && l <= 250) {
         guiGraphics.hLine(RenderType.guiOverlay(), i, i + j - 1, k - this.getSampleHeight(1.0E9D / (double)l) - 1, -16711681);
      }
   ...
   protected int getSampleHeight(double d) {
      return (int)Math.round(toMilliseconds(d) * 60.0D / 33.333333333333336D);
   }

   protected int getSampleColor(long l) {
      return this.getSampleColor(toMilliseconds((double)l), 0.0D, -16711936, 28.0D, -256, 56.0D, -65536);
   }
