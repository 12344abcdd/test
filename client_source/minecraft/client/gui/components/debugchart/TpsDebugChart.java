...
public class TpsDebugChart extends AbstractDebugChart {
   ...
   protected int getSampleHeight(double d) {
      return (int)Math.round(toMilliseconds(d) * 60.0D / (double)(Float)this.msptSupplier.get());
   }

   protected int getSampleColor(long l) {
      float f = (Float)this.msptSupplier.get();
      return this.getSampleColor(toMilliseconds((double)l), (double)f, -16711936, (double)f * 1.125D, -256, (double)f * 1.25D, -65536);
   }
