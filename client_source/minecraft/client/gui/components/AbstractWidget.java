...
public abstract class AbstractWidget implements Renderable, GuiEventListener, LayoutElement, NarratableEntry {
   ...
   public final void render(GuiGraphics guiGraphics, int i, int j, float f) {
      if (this.visible) {
         this.isHovered = guiGraphics.containsPointInScissor(i, j) && i >= this.getX() && j >= this.getY() && i < this.getX() + this.width && j < this.getY() + this.height;
   ...
   protected static void renderScrollingString(GuiGraphics guiGraphics, Font font, Component component, int i, int j, int k, int l, int m, int n) {
      int o = font.width((FormattedText)component);
      int var10000 = k + m;
      ...
      if (o > q) {
         r = o - q;
         double d = (double)Util.getMillis() / 1000.0D;
         double e = Math.max((double)r * 0.5D, 3.0D);
         double f = Math.sin(1.5707963267948966D * Math.cos(6.283185307179586D * d / e)) / 2.0D + 0.5D;
         double g = Mth.lerp(f, 0.0D, (double)r);
   ...
   protected boolean clicked(double d, double e) {
      return this.active && this.visible && d >= (double)this.getX() && e >= (double)this.getY() && d < (double)(this.getX() + this.getWidth()) && e < (double)(this.getY() + this.getHeight());
   }
