...
public class ChatComponent {
   ...
   public void render(GuiGraphics guiGraphics, int i, int j, int k, boolean bl) {
      if (!this.isChatHidden()) {
         ...
         if (m > 0) {
            ...
            double d = (Double)this.minecraft.options.chatOpacity().get() * 0.8999999761581421D + 0.10000000149011612D;
            ...
            int s = (int)Math.round(-8.0D * (g + 1.0D) + 4.0D * g);
            int t = 0;
            ...
            for(int u = 0; u + this.chatScrollbarPos < this.trimmedMessages.size() && u < l; ++u) {
               ...
               if (line != null) {
                  w = i - line.addedTime();
                  if (w < 200 || bl) {
                     double h = bl ? 1.0D : getTimeFactor(w);
                     x = (int)(255.0D * h * d);
                     y = (int)(255.0D * h * e);
                     ++t;
                     if (x > 3) {
                        ...
                        guiGraphics.drawString(this.minecraft.font, (FormattedCharSequence)line.content(), 0, ab, 16777215 + (x << 24));
            ...
            if (af > 0L) {
               ...
               guiGraphics.pose().translate(0.0F, 0.0F, 50.0F);
               guiGraphics.drawString(this.minecraft.font, (Component)Component.translatable("chat.queue", new Object[]{af}), 0, 1, 16777215 + (ag << 24));
               guiGraphics.pose().popPose();
            }
   ...
   private ChatComponent.DelayedMessageDeletion deleteMessageOrDelay(MessageSignature messageSignature) {
      int i = this.minecraft.gui.getGuiTicks();
      ListIterator listIterator = this.allMessages.listIterator();
      ...
      } while(!messageSignature.equals(guiMessage.signature()));
   ...
   public boolean handleChatQueueClicked(double d, double e) {
      if (this.isChatFocused() && !this.minecraft.options.hideGui && !this.isChatHidden()) {
         ...
         if (chatListener.queueSize() == 0L) {
            ...
            double f = d - 2.0D;
            double g = (double)this.minecraft.getWindow().getGuiScaledHeight() - e - 40.0D;
            if (f <= (double)Mth.floor((double)this.getWidth() / this.getScale()) && g < 0.0D && g > (double)Mth.floor(-9.0D * this.getScale())) {
   ...
   private double screenToChatX(double d) {
      return d / this.getScale() - 4.0D;
   }

   private double screenToChatY(double d) {
      double e = (double)this.minecraft.getWindow().getGuiScaledHeight() - d - 40.0D;
   ...
   private int getMessageLineIndexAt(double d, double e) {
      if (this.isChatFocused() && !this.isChatHidden()) {
         if (!(d < -4.0D) && !(d > (double)Mth.floor((double)this.getWidth() / this.getScale()))) {
            int i = Math.min(this.getLinesPerPage(), this.trimmedMessages.size());
            if (e >= 0.0D && e < (double)i) {
