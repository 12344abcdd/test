...
public class TabNavigationBar extends AbstractContainerEventHandler implements Renderable, NarratableEntry {
   ...
   TabNavigationBar(int i, TabManager tabManager, Iterable<Tab> iterable) {
      ...
      while(var5.hasNext()) {
         ...
         builder.add((TabButton)this.layout.addChild(new TabButton(tabManager, tab, 0, 24)));
      }
   ...
   public ComponentPath nextFocusPath(FocusNavigationEvent focusNavigationEvent) {
      if (!this.isFocused()) {
         ...
         if (tabButton != null) {
            return ComponentPath.path((ContainerEventHandler)this, (ComponentPath)ComponentPath.leaf(tabButton));
         }
      }
   ...
   protected void narrateListElementPosition(NarrationElementOutput narrationElementOutput, TabButton tabButton) {
      if (this.tabs.size() > 1) {
         ...
         if (i != -1) {
            narrationElementOutput.add(NarratedElementType.POSITION, (Component)Component.translatable("narrator.position.tab", new Object[]{i + 1, this.tabs.size()}));
         }
      }
   ...
   public void render(GuiGraphics guiGraphics, int i, int j, float f) {
      ...
      guiGraphics.blit(Screen.HEADER_SEPARATOR, 0, this.layout.getY() + this.layout.getHeight() - 2, 0.0F, 0.0F, ((TabButton)this.tabButtons.get(0)).getX(), 2, 32, 2);
