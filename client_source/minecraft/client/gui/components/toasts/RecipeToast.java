...
public class RecipeToast implements Toast {
   ...
   public Toast.Visibility render(GuiGraphics guiGraphics, ToastComponent toastComponent, long l) {
      ...
      if (this.recipes.isEmpty()) {
         ...
         guiGraphics.drawString(toastComponent.getMinecraft().font, (Component)TITLE_TEXT, 30, 7, -11534256, false);
         guiGraphics.drawString(toastComponent.getMinecraft().font, (Component)DESCRIPTION_TEXT, 30, 18, -16777216, false);
         RecipeHolder<?> recipeHolder = (RecipeHolder)this.recipes.get((int)((double)l / Math.max(1.0D, 5000.0D * toastComponent.getNotificationDisplayTimeMultiplier() / (double)this.recipes.size()) % (double)this.recipes.size()));
         ...
         return (double)(l - this.lastChanged) >= 5000.0D * toastComponent.getNotificationDisplayTimeMultiplier() ? Toast.Visibility.HIDE : Toast.Visibility.SHOW;
      }
   }
