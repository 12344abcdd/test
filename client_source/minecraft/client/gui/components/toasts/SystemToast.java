...
public class SystemToast implements Toast {
   ...
   public SystemToast(SystemToast.SystemToastId systemToastId, Component component, @Nullable Component component2) {
      this(systemToastId, component, nullToEmpty(component2), Math.max(160, 30 + Math.max(Minecraft.getInstance().font.width((FormattedText)component), component2 == null ? 0 : Minecraft.getInstance().font.width((FormattedText)component2))));
   }
   ...
   public static SystemToast multiline(Minecraft minecraft, SystemToast.SystemToastId systemToastId, Component component, Component component2) {
      ...
      int i = Math.max(200, var10001.mapToInt(font::width).max().orElse(200));
   ...
   public Toast.Visibility render(GuiGraphics guiGraphics, ToastComponent toastComponent, long l) {
      ...
      if (this.messageLines.isEmpty()) {
         guiGraphics.drawString(toastComponent.getMinecraft().font, (Component)this.title, 18, 12, -256, false);
      } else {
         guiGraphics.drawString(toastComponent.getMinecraft().font, (Component)this.title, 18, 7, -256, false);

         for(j = 0; j < this.messageLines.size(); ++j) {
            guiGraphics.drawString(toastComponent.getMinecraft().font, (FormattedCharSequence)((FormattedCharSequence)this.messageLines.get(j)), 18, 18 + j * 12, -1, false);
         }
      }
   ...
   public static void addOrUpdate(ToastComponent toastComponent, SystemToast.SystemToastId systemToastId, Component component, @Nullable Component component2) {
      SystemToast systemToast = (SystemToast)toastComponent.getToast(SystemToast.class, systemToastId);
      if (systemToast == null) {
   ...
   public static void onFileDropFailure(Minecraft minecraft, int i) {
      add(minecraft.getToasts(), SystemToast.SystemToastId.FILE_DROP_FAILURE, Component.translatable("gui.fileDropFailure.title"), Component.translatable("gui.fileDropFailure.detail", new Object[]{i}));
   }

   public static void onLowDiskSpace(Minecraft minecraft) {
      addOrUpdate(minecraft.getToasts(), SystemToast.SystemToastId.LOW_DISK_SPACE, Component.translatable("chunk.toast.lowDiskSpace"), Component.translatable("chunk.toast.lowDiskSpace.description"));
   }

   public static void onChunkLoadFailure(Minecraft minecraft, ChunkPos chunkPos) {
      addOrUpdate(minecraft.getToasts(), SystemToast.SystemToastId.CHUNK_LOAD_FAILURE, Component.translatable("chunk.toast.loadFailure", new Object[]{Component.translationArg(chunkPos)}).withStyle(ChatFormatting.RED), Component.translatable("chunk.toast.checkLog"));
   }

   public static void onChunkSaveFailure(Minecraft minecraft, ChunkPos chunkPos) {
      addOrUpdate(minecraft.getToasts(), SystemToast.SystemToastId.CHUNK_SAVE_FAILURE, Component.translatable("chunk.toast.saveFailure", new Object[]{Component.translationArg(chunkPos)}).withStyle(ChatFormatting.RED), Component.translatable("chunk.toast.checkLog"));
   }
